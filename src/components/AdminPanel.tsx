...
import { Product, Order, User, Offer, Review, PaymentSettings, Category, WeightSlab } from '../types';
import { CATEGORIES } from '../data';
import { AdminWeightSlabsEditor } from './AdminWeightSlabsEditor';
import { SELLING_UNITS, isKgProduct, getProductSellingUnit } from '../utils/weightParser';
import {
  X,
  Scale,
  TrendingUp,
...
import { AdminPaymentDiscounts } from './AdminPaymentDiscounts';
import {
  ORDER_STATUSES,
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [newProdWeightSlabs, setNewProdWeightSlabs] = useState<WeightSlab[]>([]);
  ...
  // Dedicated Quick Weight Slabs Manager Modal State
  const [slabsModalProduct, setSlabsModalProduct] = useState<Product | null>(null);
  const [slabsModalBasePrice, setSlabsModalBasePrice] = useState<number>(100);
  const [slabsModalBaseWeight, setSlabsModalBaseWeight] = useState<string>('per kg');
  const [slabsModalList, setSlabsModalList] = useState<WeightSlab[]>([]);

  const handleOpenSlabsModal = (p: Product) => {
    if (!isKgProduct(p)) {
      setAdminError(`Weight slabs are ONLY available for products sold in "kg". "${p.name}" is sold by "${p.unit || getProductSellingUnit(p)}".`);
      return;
    }
    setSlabsModalProduct(p);
    setSlabsModalBasePrice(p.sp || 100);
    setSlabsModalBaseWeight(p.weight || 'per kg');
    setSlabsModalList(p.weightSlabs ? JSON.parse(JSON.stringify(p.weightSlabs)) : []);
  };

  const handleSaveSlabsModal = () => {
    if (!slabsModalProduct) return;
    const updates: Partial<Product> = {
      sp: slabsModalBasePrice,
      weight: slabsModalBaseWeight,
      weightSlabs: slabsModalList,
      pricingMode: slabsModalList.length > 0 ? 'slabs' : 'auto',
    };
    if (onUpdateProduct) {
      onUpdateProduct(slabsModalProduct.id, updates);
    }
    setSlabsModalProduct(null);
  };
  ...
  // ── ORDER LOGISTICS & CANONICAL STATUS PIPELINE ──
  const [orderStatusSection, setOrderStatusSection] = useState<'all' | 'active' | CanonicalOrderStatus>('all');
  const [orderSearchQuery, setOrderSearchQuery] = useState('');
  const [orderPaymentFilter, setOrderPaymentFilter] = useState<'all' | 'Paid' | 'Pending' | 'COD' | 'UPI'>('all');
  const [orderViewMode, setOrderViewMode] = useState<'sections' | 'table'>('sections');
  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>({});
  const [selectedDetailsOrder, setSelectedDetailsOrder] = useState<Order | null>(null);

  // Synchronize ordersProp updates into dbTables.orders single source of truth
  useEffect(() => {
    if (ordersProp && ordersProp.length > 0) {
      setDbTables((prev: any) => ({
        ...prev,
        orders: ordersProp,
      }));
    }
  ...
  // Keep selectedDetailsOrder in sync if the order list updates
  useEffect(() => {
    if (selectedDetailsOrder) {
      const fresh = orders.find((o) => String(o.id) === String(selectedDetailsOrder.id));
      if (fresh) {
        setSelectedDetailsOrder(fresh);
      }
    }
  }, [orders]);

  const toggleSectionCollapse = (statusKey: string) => {
    setCollapsedSections((prev) => ({ ...prev, [statusKey]: !prev[statusKey] }));
  };

  const getNextStatusAction = (currentStatus: string): { next: CanonicalOrderStatus; label: string; icon: string } | null => {
    const norm = normalizeOrderStatus(currentStatus);
    switch (norm) {
      case 'New Orders':
        return { next: 'Confirmed', label: 'Confirm Order', icon: '✓' };
      case 'Confirmed':
        return { next: 'Preparing', label: 'Start Preparing', icon: '🥣' };
      case 'Preparing':
        return { next: 'Out for Delivery', label: 'Send for Delivery', icon: '🚚' };
      case 'Out for Delivery':
        return { next: 'Delivered', label: 'Mark Delivered', icon: '✅' };
      default:
        return null;
    }
  };

  // Filtered orders list based on status section, search, and payment
  // Authoritatively sorted Newest → Oldest by backend/database order creation timestamp
  const filteredOrders = useMemo(() => {
    const list = orders.filter((o) => {
      // 1. Status Filter:
      // Active orders strictly include New Orders, Confirmed, Preparing, Out for Delivery
      // and strictly EXCLUDE Delivered & Cancelled orders
      const norm = normalizeOrderStatus(o.status);
      if (orderStatusSection === 'active') {
        const isActive = ['New Orders', 'Confirmed', 'Preparing', 'Out for Delivery'].includes(norm);
        if (!isActive) return false;
      } else if (orderStatusSection !== 'all') {
        if (norm !== orderStatusSection) return false;
      }
      ...
      if (orderSearchQuery.trim()) {
        ...
        const matchesCustomer = (o.userName || '').toLowerCase().includes(q);
        ...
        const matchesItem = (o.items || []).some((it: any) =>
          (it.vegetableName || it.name || it.productName || '').toLowerCase().includes(q)
        );
        if (!matchesId && !matchesCustomer && !matchesEmail && !matchesPhone && !matchesAddress && !matchesItem) {
          return false;
        }
      }
      ...
      if (orderPaymentFilter !== 'all') {
        ...
        if (orderPaymentFilter === 'COD' && o.payment !== 'Cash on Delivery') return false;
        if (orderPaymentFilter === 'UPI' && o.payment === 'Cash on Delivery') return false;
      }

      return true;
    });

    // Authoritative Newest → Oldest sort based on actual backend creation timestamp
    return sortOrdersNewestFirst(list);
  }, [orders, orderStatusSection, orderSearchQuery, orderPaymentFilter]);

  // Group filtered orders by the 6 canonical statuses (all Newest → Oldest)
  const ordersGroupedByStatus = useMemo(() => {
    const groups: Record<CanonicalOrderStatus, Order[]> = {
      'New Orders': [],
      'Confirmed': [],
      'Preparing': [],
      'Out for Delivery': [],
    ...
    filteredOrders.forEach((o) => {
      ...
      if (groups[norm]) {
        ...
        groups['New Orders'].push(o);
      }
    });
    // Ensure every group is authoritatively sorted Newest → Oldest
    (Object.keys(groups) as CanonicalOrderStatus[]).forEach((statusKey) => {
      groups[statusKey] = sortOrdersNewestFirst(groups[statusKey]);
    });
    return groups;
  }, [filteredOrders]);

  // Global counts per status across all orders
  const statusCounts = useMemo(() => {
    const counts: Record<CanonicalOrderStatus, number> = {
      'New Orders': 0,
      'Confirmed': 0,
      'Preparing': 0,
      'Out for Delivery': 0,
  ...
  // Authoritative Order Status Transition Handler
  const handleOrderStatusUpdate = (orderId: string, newStatus: CanonicalOrderStatus) => {
    // 1. Immediately update in-memory state in dbTables.orders so all active / delivered sections update instantly
    setDbTables((prev: any) => {
    ...
    // 2. If details modal is open for this order, update it in place immediately
    ...
    // 3. Call parent callback to update App.tsx state & backend database permanently
    onUpdateOrderStatus(orderId, newStatus);
    setAdminSuccess(`Order #${orderId} moved to ${newStatus}`);
  };

  // Authoritative Order Payment Status Handler (preserves existing payment functionality)
  const handleOrderPaymentUpdate = (
        ...
        orders: prev.orders.map((o: any) =>
          String(o.id) === String(orderId)
            ? {
                ...o,
                paymentStatus,
                adminRemarks: remarks !== undefined ? remarks : o.adminRemarks,
    ...
    setSelectedDetailsOrder((prev) => {
      if (prev && String(prev.id) === String(orderId)) {
        return {
          ...prev,
          paymentStatus,
          adminRemarks: remarks !== undefined ? remarks : prev.adminRemarks,
          updatedAt: new Date().toISOString(),
        };
      }
      return prev;
    ...
    if (onUpdateOrderPaymentStatus) {
      onUpdateOrderPaymentStatus(orderId, paymentStatus, remarks);
    }
    setAdminSuccess(`Payment for Order #${orderId} marked as ${paymentStatus}`);
  };
  ...
  const handleAddProductSubmit = (e: React.FormEvent) => {
    ...
    const isKg = newProdUnit === 'kg';
    const effectiveSlabs = isKg ? newProdWeightSlabs : [];
    ...
    onAddProduct({
      ...
      cp: Number(newProdCp),
      sp: Number(newProdSp),
      basePricePerKg: Number(newProdSp),
      unit: newProdUnit,
      weight: newProdWeight || (isKg ? 'per kg' : `per ${newProdUnit}`),
      discount: '',
      ...
      lowAt: 10,
      pricingMode: isKg && effectiveSlabs.length > 0 ? 'slabs' : 'auto',
      weightSlabs: effectiveSlabs,
    });

    setNewProdName('');
    setNewProdImg('');
    setNewProdUnit('kg');
    setNewProdWeight('per kg');
    setNewProdWeightSlabs([]);
  ...
  const handleEditProductSubmit = async (e: React.FormEvent) => {
    ...
    const isKg = isKgProduct(editingProduct);
    const unit = editingProduct.unit || (isKg ? 'kg' : getProductSellingUnit(editingProduct));
    // Non-kg products must NEVER have weight slabs or custom weights
    const effectiveSlabs = isKg ? (editingProduct.weightSlabs || []) : [];
    ...
    const updates: Partial<Product> = {
      ...
      basePricePerKg: Number(editingProduct.sp),
      unit: unit,
      weight: editingProduct.weight || (isKg ? 'per kg' : `per ${unit}`),
      ...
      pricingMode: isKg && effectiveSlabs.length > 0 ? (editingProduct.pricingMode || 'slabs') : 'auto',
      weightSlabs: effectiveSlabs,
    };
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
          ...
          <aside className="w-full md:w-56 bg-emerald-950 flex flex-row md:flex-col border-b md:border-b-0 md:border-r border-emerald-800 shrink-0 text-white overflow-x-auto md:overflow-y-auto no-scrollbar">
            ...
            <nav className="flex flex-row md:flex-col flex-1 p-2 md:p-3 gap-1 md:space-y-1 overflow-x-auto md:overflow-x-visible no-scrollbar shrink-0 min-w-max md:min-w-0">
              ...
              >
                ...
                <span>Orders</span>
                {orders.filter((o) => normalizeOrderStatus(o.status) === 'New Orders').length > 0 && (
                  <span className="ml-1 rounded-full bg-emerald-600 px-1.5 py-0.5 text-[8px] font-black text-white" title="New incoming orders">
                    {orders.filter((o) => normalizeOrderStatus(o.status) === 'New Orders').length}
                  </span>
                )}
              </button>
              ...
              <button
                type="button"
                onClick={() => setActiveTab('payment-discounts')}
                className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 md:py-2 text-[11px] font-bold transition-all ${
                  activeTab === 'payment-discounts'
                    ? 'bg-emerald-800/60 text-emerald-50 border border-emerald-700/50 shadow-sm'
                    : 'text-emerald-400 hover:bg-emerald-900/50 hover:text-emerald-100'
                }`}
              >
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[var(--card)]">
            <AnimatePresence mode="wait">
                      ...
                      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-sm">
                        ...
                        <div className="overflow-x-auto max-h-[350px] overflow-y-auto">
                          <table className="w-full text-left text-xs border-collapse">
                            ...
                            <tbody className="divide-y divide-[var(--border)]">
                              {sortOrdersNewestFirst(orders).map((o: any) => {
                                ...
                                return (
                                  <tr key={o.id} className="hover:bg-gray-50/50 dark:hover:bg-zinc-900/40">
                                    <td className="py-2 font-mono font-bold text-gray-700 dark:text-gray-300">
                                      <button
                                        type="button"
                                        onClick={() => setSelectedDetailsOrder(o)}
                                        className="hover:text-[var(--primary)] hover:underline cursor-pointer flex items-center gap-1"
              ...
              {/* 2. ORDERS MANAGEMENT WITH CANONICAL STATUS SECTIONS & CONTROLS */}
              {activeTab === 'orders' && (
                <motion.div
                  key="orders"
                  initial={{ opacity: 0, y: 10 }}
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-[var(--border)]">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xl">📦</span>
                        <h3 className="text-base font-black text-[var(--fg)]">
                          Sabjies Order Logistics & Fulfillment
                        </h3>
                      </div>
                      <p className="text-xs text-[var(--muted-fg)] mt-0.5">
                        Manage active customer deliveries across the 6 canonical fulfillment stages.
                      </p>
                    </div>

                    <div className="flex items-center gap-2 flex-wrap">
                      <div className="px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-zinc-800 text-xs font-bold text-[var(--fg)] border border-[var(--border)] flex items-center gap-1.5">
                        <span>Total:</span>
                        <strong className="font-mono text-[var(--primary)]">{orders.length}</strong>
                      </div>
                      <div className="px-3 py-1.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 text-xs font-bold border border-amber-200/60 dark:border-amber-900/40 flex items-center gap-1.5">
                        <span>In Pipeline:</span>
                        <strong className="font-mono">
                          {statusCounts['New Orders'] + statusCounts['Confirmed'] + statusCounts['Preparing'] + statusCounts['Out for Delivery']}
                        </strong>
                      </div>
                      <div className="px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-300 text-xs font-bold border border-emerald-200/60 dark:border-emerald-900/40 flex items-center gap-1.5">
                        <span>Delivered:</span>
                        <strong className="font-mono">{statusCounts['Delivered']}</strong>
                      </div>
                      <div className="px-2.5 py-1 rounded-xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 text-[10px] font-extrabold border border-emerald-300/40 flex items-center gap-1">
                        <Clock className="h-3 w-3" />
                        <span>Sorted: Newest → Oldest</span>
                      </div>
                    </div>
                  </div>

                  {/* 1. High-Level Primary Pipeline Tabs (Active vs Delivered/Completed vs All) */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pb-1">
                    {/* ⚡ Active Orders Pipeline Tab */}
                    <button
                      type="button"
                      onClick={() => setOrderStatusSection('active')}
                      className={`p-3 rounded-2xl border text-left transition-all cursor-pointer relative overflow-hidden shadow-xs ${
                        orderStatusSection === 'active'
                          ? 'bg-amber-500 text-white border-amber-600 ring-2 ring-amber-400/50 shadow-md'
                          : 'bg-[var(--card)] hover:bg-amber-500/5 border-[var(--border)] text-[var(--fg)]'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-black tracking-wider flex items-center gap-1.5 opacity-90">
                          <span>⚡</span> Active Pipeline
                        </span>
                        {activePipelineCount > 0 && (
                          <span className={`h-2 w-2 rounded-full ${orderStatusSection === 'active' ? 'bg-white' : 'bg-amber-500 animate-ping'}`} />
                        )}
                      </div>
                      <div className="flex items-baseline justify-between mt-1">
                        <span className="text-xs font-bold truncate">In-Progress Orders</span>
                        <span className={`text-xs font-black font-mono px-2 py-0.5 rounded-full ${
                          orderStatusSection === 'active' ? 'bg-white/25 text-white' : 'bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-200'
                        }`}>
                          {activePipelineCount}
                        </span>
                      </div>
                      <div className="text-[9px] opacity-75 mt-0.5 truncate">
                        Stages 1–4 (Awaiting → En Route)
                      </div>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setOrderStatusSection('Delivered')}
                      className={`p-3 rounded-2xl border text-left transition-all cursor-pointer relative overflow-hidden shadow-xs ${
                        orderStatusSection === 'Delivered'
                          ? 'bg-emerald-600 text-white border-emerald-700 ring-2 ring-emerald-400/50 shadow-md'
                          : 'bg-[var(--card)] hover:bg-emerald-500/5 border-[var(--border)] text-[var(--fg)]'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-black tracking-wider flex items-center gap-1.5 opacity-90">
                          <span>🎉</span> Delivered / Completed
                        </span>
                        <span className="text-xs">✅</span>
                      </div>
                      <div className="flex items-baseline justify-between mt-1">
                        <span className="text-xs font-bold truncate">Delivered History</span>
                        <span className={`text-xs font-black font-mono px-2 py-0.5 rounded-full ${
                          orderStatusSection === 'Delivered' ? 'bg-white/25 text-white' : 'bg-emerald-100 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-200'
                        }`}>
                          {statusCounts['Delivered']}
                        </span>
                      </div>
                      <div className="text-[9px] opacity-75 mt-0.5 truncate">
                        Archived Permanently in DB
                      </div>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setOrderStatusSection('Cancelled')}
                      className={`p-3 rounded-2xl border text-left transition-all cursor-pointer relative overflow-hidden shadow-xs ${
                        orderStatusSection === 'Cancelled'
                          ? 'bg-rose-600 text-white border-rose-700 ring-2 ring-rose-400/50 shadow-md'
                          : 'bg-[var(--card)] hover:bg-rose-500/5 border-[var(--border)] text-[var(--fg)]'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-black tracking-wider flex items-center gap-1.5 opacity-90">
                      ...
                      <div className="flex items-baseline justify-between mt-1">
                        <span className="text-xs font-bold truncate">Rejected / Cancelled</span>
                        <span className={`text-xs font-black font-mono px-2 py-0.5 rounded-full ${
                          orderStatusSection === 'Cancelled' ? 'bg-white/25 text-white' : 'bg-rose-100 text-rose-900 dark:bg-rose-950 dark:text-rose-200'
                        }`}>
                    ...
                    <button
                      type="button"
                      onClick={() => setOrderStatusSection('all')}
                      className={`p-3 rounded-2xl border text-left transition-all cursor-pointer relative overflow-hidden shadow-xs ${
                        orderStatusSection === 'all'
                          ? 'bg-[var(--primary)] text-white border-[var(--primary)] ring-2 ring-[var(--primary)]/40 shadow-md'
                          : 'bg-[var(--card)] hover:bg-gray-50 dark:hover:bg-zinc-900 border-[var(--border)] text-[var(--fg)]'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-black tracking-wider flex items-center gap-1.5 opacity-90">
                      ...
                      <div className="flex items-baseline justify-between mt-1">
                        <span className="text-xs font-bold truncate">Total Ledger</span>
                        <span className={`text-xs font-black font-mono px-2 py-0.5 rounded-full ${
                          orderStatusSection === 'all' ? 'bg-white/25 text-white' : 'bg-[var(--muted)] text-[var(--fg)]'
                        }`}>
                          {orders.length}
                        </span>
                      </div>
                  ...
                  {/* 2. Granular Canonical Status Sub-Stages (1 through 6) */}
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                    {ORDER_STATUSES.map((statusKey, idx) => {
                      const meta = getOrderStatusMeta(statusKey);
                      const count = statusCounts[statusKey];
                      const isSelected = orderStatusSection === statusKey;

                      return (
                        <button
                          key={statusKey}
                          type="button"
                          onClick={() => setOrderStatusSection(statusKey)}
                          className={`p-2 rounded-xl border text-left transition-all cursor-pointer relative overflow-hidden ${
                            isSelected
                              ? `${meta.badgeClass} ring-2 ring-[var(--primary)]/40 shadow-sm`
                              : 'bg-[var(--card)] hover:bg-gray-50 dark:hover:bg-zinc-900 border-[var(--border)] text-[var(--fg)]'
                          }`}
                        >
                          <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-baseline justify-between mt-1">
                            <span className={`text-[10px] font-semibold truncate ${isSelected ? 'opacity-90' : 'text-[var(--muted-fg)]'}`}>
                              {statusKey === 'New Orders' ? 'Awaiting Action' : statusKey === 'Preparing' ? 'In Packing' : statusKey === 'Delivered' ? 'Completed' : statusKey}
                            </span>
                            <span className={`text-xs font-black font-mono px-1.5 py-0.5 rounded-full ${
                              isSelected
                                ? 'bg-black/10 dark:bg-white/15'
                                : count > 0 && (statusKey === 'New Orders' || statusKey === 'Preparing')
                                ? 'bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-200'
                                : count > 0 && statusKey === 'Delivered'
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3 rounded-2xl bg-[var(--card)] border border-[var(--border)]">
                    {/* Search query */}
                    <div className="relative flex-1">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[var(--muted-fg)]" />
                      <input
                        type="text"
                        value={orderSearchQuery}
                        onChange={(e) => setOrderSearchQuery(e.target.value)}
                        placeholder="Search by Order #, Customer, Phone, Address, or Item..."
                        className="w-full pl-9 pr-8 py-1.5 rounded-xl border border-[var(--border)] bg-[var(--bg)] text-xs text-[var(--fg)] placeholder-[var(--muted-fg)] outline-none focus:border-[var(--primary)]"
                      />
                      {orderSearchQuery && (
                        <button
                          type="button"
                          onClick={() => setOrderSearchQuery('')}
                          className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-[var(--muted-fg)] hover:text-[var(--fg)]"
                        >
                          ✕
                        </button>
                      )}
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        className="rounded-xl border border-[var(--border)] bg-[var(--bg)] px-3 py-1.5 text-xs font-semibold text-[var(--fg)] outline-none"
                      >
                      ...
                      {/* View mode toggle */}
                      <div className="flex items-center rounded-xl border border-[var(--border)] bg-[var(--bg)] p-0.5">
                        <button
                          type="button"
                          onClick={() => setOrderViewMode('sections')}
                          className={`px-3 py-1 rounded-lg text-[11px] font-bold transition-all ${
                            orderViewMode === 'sections'
                  ...
                  {/* 3. ORDER SECTIONS VIEW (Grouped by the 6 canonical statuses) */}
                  {orderViewMode === 'sections' ? (
                    <div className="space-y-4">
                      {(
                        orderStatusSection === 'all'
                          ? ORDER_STATUSES
                          : orderStatusSection === 'active'
                          ? (['New Orders', 'Confirmed', 'Preparing', 'Out for Delivery'] as CanonicalOrderStatus[])
                          : [orderStatusSection]
                      ).map((statusKey, statusIdx) => {
                        const meta = getOrderStatusMeta(statusKey);
                        const sectionOrders = ordersGroupedByStatus[statusKey] || [];
                        const isCollapsed = collapsedSections[statusKey] || false;

                        return (
                          <div
                            key={statusKey}
                            className="rounded-2xl border border-[var(--border)] bg-[var(--card)] overflow-hidden shadow-xs"
                          >
                            {/* Section Header */}
                            <div
                              onClick={() => toggleSectionCollapse(statusKey)}
                              className={`p-3.5 flex items-center justify-between cursor-pointer border-b transition-colors ${
                                sectionOrders.length > 0
                                  ? 'bg-gray-50/70 dark:bg-zinc-900/60 border-[var(--border)] hover:bg-gray-100/70 dark:hover:bg-zinc-800/60'
                                  : 'bg-[var(--card)] border-[var(--border)] opacity-80'
                              }`}
                            >
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <div className="flex items-center gap-2">
                                    <h4 className="text-xs font-black text-[var(--fg)] uppercase tracking-wide">
                                      {statusIdx + 1}. {statusKey}
                                    </h4>
                                  ...
                                  <p className="text-[10px] text-[var(--muted-fg)] mt-0.5 font-medium">
                                    {meta.description}
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {isCollapsed ? (
                                  <ChevronDown className="h-4 w-4 text-[var(--muted-fg)]" />
                                ) : (
                                  <ChevronUp className="h-4 w-4 text-[var(--muted-fg)]" />
                                )}
                              </div>
                            </div>
                            ...
                            {!isCollapsed && (
                              <div className="p-3">
                                {sectionOrders.length === 0 ? (
                                  <div className="text-center py-6 text-[var(--muted-fg)]">
                                    <p className="text-xs font-bold text-[var(--fg)]">No orders in "{statusKey}"</p>
                                    <p className="text-[10px] mt-0.5 text-[var(--muted-fg)]">
                                      Orders will appear here as they transition into the {statusKey.toLowerCase()} stage.
                                    </p>
                                  </div>
                                ) : (
                                  <div className="space-y-3">
                                    {sectionOrders.map((o, oIdx) => {
                                      const nextAction = getNextStatusAction(o.status);
                                      const currentNorm = normalizeOrderStatus(o.status);

                                      return (
                                        <div
                                          key={o.id}
                                          className="rounded-xl border border-[var(--border)] bg-[var(--bg)] p-3.5 space-y-3 hover:border-gray-300 dark:hover:border-zinc-700 transition-all shadow-2xs"
                                        >
                                          {/* Top Info Bar */}
                                          <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-[var(--border)]">
                                            <div className="flex items-center gap-2">
                                              <button
                                                type="button"
                                                onClick={() => setSelectedDetailsOrder(o)}
                                                className="font-mono font-black text-xs text-[var(--primary)] hover:underline cursor-pointer flex items-center gap-1 group"
                                                title="Click to open full order, customer, items & total breakdown"
                                              >
                                                <span>#{o.id}</span>
                                                <Eye className="h-3 w-3 opacity-70 group-hover:opacity-100 transition-opacity text-[var(--muted-fg)]" />
                                              </button>
                                              {oIdx === 0 && (
                                                <span className="inline-flex items-center gap-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950/60 px-2 py-0.5 text-[9px] font-black text-emerald-800 dark:text-emerald-300 border border-emerald-300/60 dark:border-emerald-800">
                                                  ⚡ Top
                                                </span>
                                              )}
                                              <span className="text-[10px] text-[var(--muted-fg)] font-medium flex items-center gap-1">
                                                <Clock className="h-3 w-3" />
                                            ...
                                            {/* Status Badge & Direct Status Selector */}
                                            <div className="flex items-center gap-2">
                                              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wide border shadow-2xs ${meta.badgeClass}`}>
                                              ...
                                              <div className="flex items-center gap-1">
                                                ...
                                                <select
                                                  value={currentNorm}
                                                  onChange={(e) => {
                                                    const newStatus = e.target.value as CanonicalOrderStatus;
                                                    handleOrderStatusUpdate(o.id, newStatus);
                                                  }}
                                                  className="rounded-lg border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-[11px] font-bold text-[var(--fg)] outline-none focus:ring-1 focus:ring-[var(--primary)]"
                                                >
                                          ...
                                          {/* Middle Grid: Customer, Items, Logistics & Payment */}
                                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                                            {/* Customer & Address Details */}
                                            <div className="space-y-1">
                                              <span className="text-[10px] font-extrabold uppercase tracking-wider text-[var(--muted-fg)] block">
                                                Customer Information
                                              </span>
                                              <div className="font-bold text-[var(--fg)] text-xs">{o.userName || 'Customer'}</div>
                                              <div className="text-[10px] text-[var(--muted-fg)] truncate">{o.userEmail}</div>
                                              {o.phone && (
                                                <div className="text-[10px] text-[var(--muted-fg)] flex items-center gap-1">
                                                  <Phone className="h-3 w-3 text-emerald-600" />
                                                  <a href={`tel:${o.phone}`} className="hover:underline font-mono">
                                                    {o.phone}
                                                  </a>
                                                </div>
                                              )}
                                              <div className="text-[10px] text-[var(--muted-fg)] flex items-start gap-1 pt-1">
                                                <MapPin className="h-3 w-3 text-rose-500 shrink-0 mt-0.5" />
                                                <span className="line-clamp-2">{o.address || 'Address on file'}</span>
                                              </div>
                                            </div>
                                            ...
                                            <div className="space-y-1">
                                              <span className="text-[10px] font-extrabold uppercase tracking-wider text-[var(--muted-fg)] block">
                                                Purchased Produce ({o.items?.length || 0})
                                              </span>
                                              <div className="space-y-1 max-h-24 overflow-y-auto pr-1">
                                                {(o.items || []).map((it: any, itemIdx: number) => {
                                                  ...
                                                  return (
                                                    <div key={itemIdx} className="text-[11px] leading-tight flex items-center justify-between gap-1 py-0.5">
                                                      <span className="font-semibold text-[var(--fg)] truncate">
                                                        {it.emoji || '🥬'} {clean}{weight}
                                                      </span>
                                                      <span className="font-mono text-[10px] text-[var(--muted-fg)] shrink-0 font-bold">
                                                        {qty > 1 ? `${qty} × ₹${unitPrice} = ₹${itemTotal}` : `₹${itemTotal}`}
                                                      </span>
                                                    </div>
                                                  );
                                                })}
                                            ...
                                            <div className="space-y-1">
                                              ...
                                              <div className="flex items-center justify-between">
                                                <span className="text-xs font-bold text-[var(--fg)]">
                                                  Total: <strong className="font-mono text-sm text-[var(--primary)]">₹{o.total}</strong>
                                                </span>
                                              ...
                                              <div className="text-[10px] text-[var(--muted-fg)]">
                                                Mode: <span className="font-semibold text-[var(--fg)]">{o.paymentGateway === 'razorpay' || o.razorpayPaymentId ? 'Razorpay Online' : o.payment}</span>
                                              </div>
                                              {(o.transactionId || o.razorpayOrderId) && (
                                                <div className="text-[9px] font-mono text-[var(--muted-fg)] truncate" title={o.transactionId || o.razorpayOrderId}>
                                                  Ref: {o.transactionId || o.razorpayOrderId}
                                                </div>
                                              )}
                                              ...
                                              <div className="flex items-center gap-1.5 pt-1.5 flex-wrap">
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() => setSelectedDetailsOrder(o)}
                                                  className="px-2.5 py-1 rounded-lg bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 text-[var(--primary)] border border-[var(--primary)]/20 text-[10px] font-extrabold flex items-center gap-1 transition-all cursor-pointer shadow-xs"
                                                  title="Open full customer, order, items & total breakdown"
                                                >
                                                ...
                                                <button
                                                  ...
                                                  className="px-2.5 py-1 rounded-lg border border-[var(--border)] bg-[var(--card)] hover:bg-gray-100 dark:hover:bg-zinc-800 text-[10px] font-bold text-[var(--fg)] flex items-center gap-1 transition-all cursor-pointer"
                                                >
                                                  <Printer className="h-3 w-3 text-gray-500" />
                                                  <span>Invoice</span>
                                                </button>

                                                {/* Quick Next Stage Workflow Action Button */}
                                                {nextAction && (
                                                  <button
                                                    type="button"
                                                    onClick={() => {
                                                      handleOrderStatusUpdate(o.id, nextAction.next);
                                                    }}
                                                    className="px-2.5 py-1 rounded-lg bg-[var(--primary)] hover:opacity-90 text-white text-[10px] font-extrabold flex items-center gap-1 transition-all shadow-xs cursor-pointer"
                                                  >
                                                ...
                                                {/* Quick Cancel Button if not already cancelled or delivered */}
                                                {currentNorm !== 'Cancelled' && currentNorm !== 'Delivered' && (
                                                  <button
                                                    type="button"
                                                    onClick={() => {
                                                      if (window.confirm(`Are you sure you want to mark Order #${o.id} as Cancelled?`)) {
                                                        onUpdateOrderStatus(o.id, 'Cancelled');
                                                        setAdminSuccess(`Order #${o.id} marked as Cancelled.`);
                                                      }
                                                    }}
                                                    className="px-2 py-1 rounded-lg border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-300 text-[10px] font-bold hover:bg-red-100 dark:hover:bg-red-900 transition-all cursor-pointer"
                                                  >
                                                    Cancel
                                                  </button>
                                                )}
                    ...
                    /* 4. COMPREHENSIVE DATA TABLE VIEW */
                      ...
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="bg-gray-50 dark:bg-zinc-900 border-b border-[var(--border)] text-[var(--muted-fg)] uppercase tracking-wider text-[10px] font-bold">
                            ...
                            <th className="p-3 min-w-[190px]">Status & Pipeline Control</th>
                          ...
                          {filteredOrders.map((o, oIdx) => {
                            ...
                            return (
                              <tr key={o.id} className="hover:bg-gray-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                                <td className="p-3 font-mono font-extrabold text-[var(--primary)] whitespace-nowrap">
                                  <div className="flex items-center gap-1.5">
                                    <button
                                      type="button"
                                      onClick={() => setSelectedDetailsOrder(o)}
                                      className="hover:underline cursor-pointer flex items-center gap-1 text-left"
                                      title="Open order details modal"
                                    >
                                      <span>#{o.id}</span>
                                      <Eye className="h-3 w-3 opacity-60 hover:opacity-100 text-[var(--muted-fg)]" />
                                    </button>
                                    {oIdx === 0 && (
                                      <span className="inline-flex items-center gap-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950/60 px-1.5 py-0.5 text-[8px] font-black text-emerald-800 dark:text-emerald-300 border border-emerald-300/60">
                                        ⚡ Top
                                      </span>
                                    )}
                                  </div>
                                  <div className="text-[9px] text-[var(--muted-fg)] font-sans font-normal mt-0.5 flex items-center gap-1">
                                ...
                                <td className="p-3 max-w-[140px]">
                                  ...
                                  <div className="text-[10px] text-[var(--muted-fg)] truncate">{o.userEmail}</div>
                                  {o.phone && (
                                    <div className="text-[10px] text-[var(--muted-fg)] font-mono">{o.phone}</div>
                                  )}
                                </td>
                                <td className="p-3 max-w-[220px] text-[var(--muted-fg)]">
                                  <div className="space-y-1">
                                    {(o.items || []).map((it: any, itemIdx: number) => {
                                      const raw = it.vegetableName || it.name || it.productName || it.title || it.itemName || 'Product';
                                      const clean = (raw === '()' || raw === '( )') ? 'Product' : raw;
                                      ...
                                      const itemTotal = it.itemTotal !== undefined ? it.itemTotal : (it.lineTotal !== undefined ? it.lineTotal : (unitPrice * qty));
                                      return (
                                        <div key={itemIdx} className="text-[11px] leading-tight flex items-center justify-between gap-1">
                                          <span className="font-semibold text-[var(--fg)] truncate">{it.emoji || '🥬'} {clean}{weight}</span>
                                          <span className="text-[10px] text-[var(--muted-fg)] shrink-0 font-mono">
                                            {qty > 1 ? `${qty}×₹${unitPrice}` : `₹${itemTotal}`}
                                          </span>
                                        </div>
                                      );
                                    })}
                                ...
                                <td className="p-3 font-bold text-[var(--fg)] font-mono whitespace-nowrap">
                                  ₹{o.total}
                                </td>
                                <td className="p-3 space-y-1 whitespace-nowrap">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className="font-bold text-[10px] uppercase text-[var(--fg)]">
                                      {o.paymentGateway === 'razorpay' || o.razorpayPaymentId ? 'Razorpay' : o.payment === 'Cash on Delivery' ? 'COD' : 'UPI'}
                                    </span>
                                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-extrabold uppercase ${
                                      o.paymentStatus === 'Paid' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200' :
                                      o.paymentStatus === 'Pending' ? 'bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-200' :
                                  ...
                                  {(o.transactionId || o.razorpayOrderId) && (
                                    <div className="text-[9px] text-[var(--muted-fg)] font-mono truncate max-w-[130px]" title={o.transactionId || o.razorpayOrderId}>
                                      Ref: {o.transactionId || o.razorpayOrderId}
                                    </div>
                                  )}
                                  {(o.razorpayPaymentId || o.paymentGateway === 'razorpay' || (o.payment || '').toLowerCase().includes('razorpay')) && o.paymentStatus === 'Paid' && (
                                ...
                                <td className="p-3 space-y-2">
                                  <div className="flex items-center gap-1">
                                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wide border ${meta.badgeClass}`}>
                                      <span className={`h-1.5 w-1.5 rounded-full ${meta.dotClass}`} />
                                      <span>{meta.label}</span>
                                    </span>
                                  </div>

                                  {/* Canonical Order Status Selector */}
                                  <select
                                    value={currentNorm}
                                    onChange={(e) => {
                                      const newStatus = e.target.value as CanonicalOrderStatus;
                                    ...
                                    className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-1.5 text-[11px] font-bold text-[var(--fg)] outline-none w-full focus:ring-1 focus:ring-[var(--primary)]"
                                  >
                                    {ORDER_STATUSES.map((st, sIdx) => (
                                      <option key={st} value={st}>
                                        {sIdx + 1}. {st}
                                  ...
                                  {nextAction && (
                                    <button
                                      ...
                                      className="flex items-center justify-center gap-1 w-full px-2 py-1 rounded-lg bg-[var(--primary)] text-white text-[9px] font-extrabold hover:opacity-90 transition-all cursor-pointer"
                                    >
                                      <span>{nextAction.icon}</span>
                                      <span>{nextAction.label}</span>
                                    </button>
                                ...
                                <td className="p-3 space-y-1.5 whitespace-nowrap">
                                  <button
                                    type="button"
                                    onClick={() => setSelectedDetailsOrder(o)}
                                    className="flex items-center justify-center gap-1 w-full px-2.5 py-1 rounded-lg bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 text-[var(--primary)] border border-[var(--primary)]/20 text-[10px] font-extrabold transition-all cursor-pointer shadow-xs"
                                    title="Open customer, items, and financial summary"
                                  >
                                    <Eye className="h-3 w-3" />
                                    <span>Open Order</span>
                                  </button>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setSelectedInvoiceOrder(o)}
                                    className="flex items-center justify-center gap-1 w-full px-2.5 py-1 rounded-lg border border-[var(--border)] bg-[var(--muted)] text-[10px] font-bold text-[var(--primary)] hover:bg-emerald-50 dark:hover:bg-emerald-950/20 transition-all cursor-pointer"
                                  >
                                  ...
                                  {currentNorm !== 'Cancelled' && currentNorm !== 'Delivered' && (
                                    <button
                                      ...
                                      onClick={() => {
                                        if (window.confirm(`Are you sure you want to mark Order #${o.id} as Cancelled?`)) {
                                          handleOrderStatusUpdate(o.id, 'Cancelled');
                                        }
                                      }}
                                      className="flex items-center justify-center gap-1 w-full px-2 py-0.5 rounded-lg border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-300 text-[9px] font-bold hover:bg-red-100 transition-all cursor-pointer"
                                    >
                                      Cancel
                                    </button>
                                  )}
              ...
              {activeTab === 'products' && (
                ...
                >
                  ...
                  <AnimatePresence>
                    {showAddProduct && (
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold text-[var(--muted-fg)] uppercase">Selling Unit *</label>
                          <select
                            value={newProdUnit}
                            onChange={(e) => {
                              const selectedUnit = e.target.value;
                              setNewProdUnit(selectedUnit);
                              if (selectedUnit === 'kg') {
                                setNewProdWeight('per kg');
                              } else {
                                // Clear weight slabs immediately when switching to non-kg
                                setNewProdWeightSlabs([]);
                                const opt = SELLING_UNITS.find(u => u.value === selectedUnit);
                                setNewProdWeight(opt ? opt.defaultMetric : `per ${selectedUnit}`);
                              }
                            }}
                            className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-xs text-[var(--fg)] outline-none font-bold"
                          >
                            {SELLING_UNITS.map(opt => (
                              <option key={opt.value} value={opt.value}>
                                {opt.label} {opt.allowsWeightCatalog ? '⚖️ (Weight Slabs)' : '📦'}
                              </option>
                            ))}
                          </select>
                        </div>
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold text-[var(--muted-fg)] uppercase">Weight Metric / Label</label>
                          <input
                            type="text"
                            required
                            value={newProdWeight || ''}
                            onChange={(e) => setNewProdWeight(e.target.value)}
                            placeholder={newProdUnit === 'kg' ? 'per kg' : `per ${newProdUnit}`}
                            className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-4.5 py-2 text-xs text-[var(--fg)] outline-none"
                          />
                        </div>
                        ...
                        {/* Vegetable Pricing Engine: Active ONLY when Selling Unit is 'kg' */}
                        <div className="sm:col-span-3">
                          {newProdUnit === 'kg' ? (
                            <AdminWeightSlabsEditor
                              basePrice={Number(newProdSp) || 1}
                              baseWeight={newProdWeight || 'per kg'}
                              unit={newProdUnit}
                              weightSlabs={newProdWeightSlabs}
                              onBasePriceChange={(val) => setNewProdSp(val)}
                              onBaseWeightChange={(val) => setNewProdWeight(val)}
                              onWeightSlabsChange={(slabs) => setNewProdWeightSlabs(slabs)}
                              productName={newProdName || 'New Vegetable'}
                              productEmoji={newProdEmoji || '🥦'}
                            />
                          ) : (
                            <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--muted)]/40 p-5 text-center">
                              <div className="flex items-center justify-center gap-2 text-xs font-bold text-[var(--fg)]">
                                <Scale className="h-4 w-4 text-[var(--muted-fg)]" />
                                <span>Weight Management Disabled for Non-kg Products</span>
                              </div>
                              <p className="mt-1 text-[11px] text-[var(--muted-fg)] max-w-md mx-auto">
                                Selling unit is set to <strong>"{newProdUnit}"</strong>. Weight slabs, custom gram rates (250g, 500g, 750g), and fractional weight options only apply when the selling unit is <strong>“kg”</strong>.
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            {/* Slabs or Auto-Pricing indicator (kg only) vs Non-kg badge */}
                            {!isKgProduct(p) ? (
                              <span
                                className="rounded-full bg-gray-100 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 px-2 py-0.5 text-[9px] font-semibold text-gray-600 dark:text-gray-400 flex items-center gap-1 cursor-default"
                                title={`Sold per ${p.unit || getProductSellingUnit(p)}. Weight management is disabled for non-kg products.`}
                              >
                                <Package className="h-2.5 w-2.5" />
                                <span>Unit: {p.unit || getProductSellingUnit(p)}</span>
                              </span>
                            ) : p.weightSlabs && p.weightSlabs.length > 0 ? (
                              <button
                                type="button"
                                onClick={() => handleOpenSlabsModal(p)}
                                className="rounded-full bg-amber-50 dark:bg-amber-950/50 border border-amber-300 dark:border-amber-700 px-2 py-0.5 text-[9px] font-black text-amber-700 dark:text-amber-300 flex items-center gap-1 hover:bg-amber-100 dark:hover:bg-amber-900/50 transition-colors cursor-pointer"
                                title="Custom weight slabs active. Click to manage slabs."
                              >
                                <Layers className="h-2.5 w-2.5" />
                                <span>⭐ {p.weightSlabs.length} Slabs</span>
                              </button>
                            ) : (
                              <button
                                type="button"
                                onClick={() => handleOpenSlabsModal(p)}
                                className="rounded-full bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-300 dark:border-emerald-700 px-2 py-0.5 text-[9px] font-bold text-emerald-700 dark:text-emerald-300 flex items-center gap-1 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors cursor-pointer"
                                title="Auto calculated from base price. Click to configure weight slabs."
                              >
                        ...
                        <form onSubmit={handleEditProductSubmit} className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                            ...
                            <div className="flex flex-col gap-1">
                              <label className="text-[10px] font-bold text-[var(--muted-fg)] uppercase">Selling Unit *</label>
                              <select
                                value={editingProduct.unit || (isKgProduct(editingProduct) ? 'kg' : getProductSellingUnit(editingProduct))}
                                onChange={(e) => {
                                  const newUnit = e.target.value;
                                  const isNowKg = newUnit === 'kg';
                                  if (isNowKg) {
                                    // If changed back to kg, allow Weight Management again
                                    setEditingProduct({
                                      ...editingProduct,
                                      unit: 'kg',
                                      weight: editingProduct.weight && editingProduct.weight.toLowerCase().includes('kg') ? editingProduct.weight : 'per kg'
                                    });
                                  } else {
                                    // Remove/clear any previously saved weight catalog data if admin changes from kg -> another unit
                                    const opt = SELLING_UNITS.find(u => u.value === newUnit);
                                    setEditingProduct({
                                      ...editingProduct,
                                      unit: newUnit,
                                      weight: opt ? opt.defaultMetric : `per ${newUnit}`,
                                      weightSlabs: [],
                                      pricingMode: 'auto'
                                    });
                                  }
                                }}
                                className="rounded-xl border border-[var(--border)] bg-[var(--bg)] px-2 py-2 text-xs text-[var(--fg)] outline-none font-bold"
                              >
                                {SELLING_UNITS.map(opt => (
                                  <option key={opt.value} value={opt.value}>
                                    {opt.label} {opt.allowsWeightCatalog ? '⚖️ (Weight Slabs)' : '📦'}
                            ...
                            <div className="flex flex-col gap-1 col-span-2 sm:col-span-1">
                              ...
                              <input
                                ...
                                placeholder={isKgProduct(editingProduct) ? 'per kg' : `per ${editingProduct.unit || 'piece'}`}
                                className="rounded-xl border border-[var(--border)] bg-[var(--bg)] px-3 py-2 text-xs text-[var(--fg)] outline-none"
                              />
                            </div>
                          </div>
                          ...
                          {/* Vegetable Pricing Engine: Active ONLY when Selling Unit is 'kg' */}
                          {isKgProduct(editingProduct) ? (
                            <AdminWeightSlabsEditor
                              basePrice={Number(editingProduct.sp) || 1}
                              baseWeight={editingProduct.weight || 'per kg'}
                              unit={editingProduct.unit || 'kg'}
                              weightSlabs={editingProduct.weightSlabs || []}
                              onBasePriceChange={(val) => setEditingProduct({ ...editingProduct, sp: val })}
                              onBaseWeightChange={(val) => setEditingProduct({ ...editingProduct, weight: val })}
                              onWeightSlabsChange={(slabs) => setEditingProduct({ ...editingProduct, weightSlabs: slabs, pricingMode: slabs.length > 0 ? 'slabs' : 'auto' })}
                              productName={editingProduct.name || 'Vegetable'}
                              productEmoji={editingProduct.emoji || '🥦'}
                            />
                          ) : (
                            <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--muted)]/40 p-5 text-center">
                              <div className="flex items-center justify-center gap-2 text-xs font-bold text-[var(--fg)]">
                              ...
                              <p className="mt-1 text-[11px] text-[var(--muted-fg)] max-w-md mx-auto">
                                This product is sold by <strong>"{editingProduct.unit || getProductSellingUnit(editingProduct)}"</strong>. All weight slabs and custom gram weights have been cleared. Weight Management only activates when <strong>“kg”</strong> is selected.
                              </p>
                            </div>
                          )}
                  ...
                  {/* Quick Dedicated Weight-Price Slabs Manager Modal */}
                  {slabsModalProduct && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
                      <motion.div
                        ...
                        animate={{ scale: 1, opacity: 1 }}
                        exit={{ scale: 0.95, opacity: 0 }}
                        className="relative w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-2xl bg-[var(--card)] p-4 sm:p-6 shadow-2xl border border-[var(--border)] space-y-4"
                      >
                        <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
                          <div className="flex items-center gap-2.5">
                            <span className="text-2xl">{slabsModalProduct.emoji || '🥦'}</span>
                            <div>
                              <h3 className="text-base font-black text-[var(--fg)]">
                                Manage Weight Slabs: {slabsModalProduct.name}
                              </h3>
                              <p className="text-xs text-[var(--muted-fg)]">
                                Configure base rate and custom weight pricing slabs (highest priority).
                              </p>
                            </div>
                          </div>
                          <button
                            type="button"
                            onClick={() => setSlabsModalProduct(null)}
                            className="p-1.5 text-gray-400 hover:text-[var(--fg)] rounded-lg hover:bg-[var(--muted)] cursor-pointer"
                          >
                            <X className="h-5 w-5" />
                          </button>
                        </div>
                        ...
                        <AdminWeightSlabsEditor
                          basePrice={slabsModalBasePrice}
                          baseWeight={slabsModalBaseWeight}
                          unit={slabsModalProduct.unit || 'kg'}
                          weightSlabs={slabsModalList}
                          onBasePriceChange={(val) => setSlabsModalBasePrice(val)}
                          onBaseWeightChange={(val) => setSlabsModalBaseWeight(val)}
                          onWeightSlabsChange={(slabs) => setSlabsModalList(slabs)}
                          productName={slabsModalProduct.name}
                          productEmoji={slabsModalProduct.emoji}
                        />
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-[var(--border)]">
                          <button
                            type="button"
                            onClick={() => setSlabsModalProduct(null)}
                            className="rounded-xl border border-[var(--border)] px-4 py-2 text-xs font-bold text-[var(--muted-fg)] hover:bg-[var(--muted)] cursor-pointer"
                          >
                            Cancel
                          </button>
                          <button
                            type="button"
                            onClick={handleSaveSlabsModal}
                            className="rounded-xl bg-[var(--primary)] px-5 py-2 text-xs font-black text-white shadow hover:opacity-95 flex items-center gap-1.5 cursor-pointer"
                          >
                            <Check className="h-4 w-4" />
                            <span>Save Slabs & Rate</span>
                          </button>
                        </div>
                      </motion.div>
                    </div>
              ...
              {/* PAYMENT METHOD DISCOUNTS VIEW */}
              {activeTab === 'payment-discounts' && (
                <motion.div
                  key="payment-discounts"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="space-y-6"
                >
                  <AdminPaymentDiscounts
              ...
              {selectedDetailsOrder && (
                <AdminOrderDetailsModal
                  ...
                  onUpdatePaymentStatus={handleOrderPaymentUpdate}
                  onOpenInvoice={(ord) => setSelectedInvoiceOrder(ord)}
                  onInitiateRazorpayRefund={handleInitiateRazorpayRefund}
                />
              )}
