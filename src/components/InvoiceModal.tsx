...
import { Order, PaymentSettings, BusinessSettings } from '../types';
...
export const InvoiceModal: React.FC<InvoiceModalProps> = ({ order, onClose }) => {
  ...
  const couponSavings = order.discountApplied || 0;
  const paymentSavings = order.paymentDiscountAmount || 0;
  const totalSavings = order.totalDiscount !== undefined ? order.totalDiscount : (couponSavings + paymentSavings);
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-slate-100 pb-6 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] md:text-xs text-slate-500 font-medium mt-1">{businessSettings.address}</p>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 font-black text-slate-500">
                  <th className="py-3 px-4">Vegetable & Weight</th>
            ...
            <div className="space-y-2 text-xs text-slate-600 font-medium">
              ...
              {couponSavings > 0 && (
                <div className="flex justify-between text-emerald-600 font-bold">
                  ...
                  <span className="font-mono">- ₹{couponSavings}</span>
                </div>
              )}
              ...
              {paymentSavings > 0 && (
                <div className="flex justify-between text-emerald-600 font-bold">
                  <span className="flex items-center gap-1">
                    <CreditCard className="h-3.5 w-3.5" /> Payment Method Discount ({order.paymentDiscountLabel || order.payment || 'Special'}):
                  </span>
                  <span className="font-mono">- ₹{paymentSavings}</span>
                </div>
              )}
