...
import { X, Star, Heart, ShoppingBag, Plus, Minus, Share2, ShieldCheck, Truck, Check, Scale, Sparkles } from 'lucide-react';
...
import { POPULAR_CUSTOM_SUGGESTIONS } from './CustomWeightModal';
...
export const ProductDetailsModal: React.FC<ProductDetailsModalProps> = ({
  ...
  // Custom Weight Calculation & State
  ...
  const [weightInput, setWeightInput] = useState<string>(formatWeight(baseGrams));
  const [unitMode, setUnitMode] = useState<'g' | 'kg'>(baseGrams >= 1000 ? 'kg' : 'g');
  const [isCustomInputOpen, setIsCustomInputOpen] = useState(false);

  // Synchronize when product changes
  useEffect(() => {
    if (product) {
      const g = getBaseWeightInGrams(product.weight);
      setSelectedWeightGrams(g);
      setWeightInput(formatWeight(g));
      setUnitMode(g >= 1000 ? 'kg' : 'g');
      setIsCustomInputOpen(false);
    }
  }, [product?.id]);

  // Parse weight in real-time
  const parsedWeight = useMemo(() => {
    return parseWeightToGrams(weightInput, unitMode);
  }, [weightInput, unitMode]);

  // Instant price calculation without page refresh, with custom slab priority
  const priceResult = useMemo(() => {
    if (!product) return null;
    const grams = parsedWeight.isValid && parsedWeight.grams > 0 ? parsedWeight.grams : selectedWeightGrams;
    return calculateWeightPrice(product.sp, product.weight, grams, product.weightSlabs);
  }, [product, parsedWeight, selectedWeightGrams]);

  const isKg = product ? isKgProduct(product) : false;
  const activePrice = isKg && priceResult ? priceResult.price : (product?.sp || 0);
  const activeDisplayWeight = isKg && priceResult ? priceResult.displayWeight : (product?.weight || '1 unit');
  const activeGrams = isKg && priceResult ? priceResult.grams : baseGrams;

  const handleSelectPreset = (grams: number) => {
    setSelectedWeightGrams(grams);
    setWeightInput(formatWeight(grams));
    setUnitMode(grams >= 1000 ? 'kg' : 'g');
    setIsCustomInputOpen(false);
  };
  ...
  const handleUnitToggle = (unit: 'g' | 'kg') => {
    ...
    if (unit === 'kg') {
      ...
      setWeightInput(`${kgVal}kg`);
    } else {
      setWeightInput(`${parsedWeight.grams}g`);
    }
  };
            ...
            <div data-lenis-prevent className="flex-1 overflow-y-auto p-5 md:p-6 flex flex-col justify-between overscroll-contain">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--muted-fg)] mt-1.5 font-bold uppercase flex items-center gap-1.5">
                    <span>Base Rate: ₹{product.sp} per {product.weight}</span>
                    {isKg && activeGrams !== baseGrams && (
                      <span className="text-emerald-600 dark:text-emerald-400 font-extrabold">
                        • Selected: {activeDisplayWeight}
                      </span>
                    )}
                  </p>
                </div>

                {/* Live Cost Section - Updates Instantly on Weight Change */}
                <div className="flex flex-wrap items-baseline gap-3 pt-1">
                  <span className="text-3xl font-black text-[var(--primary)] transition-all duration-200">
                    ₹{activePrice}
                  </span>
                  <span className="text-xs font-extrabold text-[var(--muted-fg)] uppercase">
                    {isKg ? `for ${activeDisplayWeight}` : `per ${product.unit || product.weight}`}
                  </span>
                  {isKg && priceResult?.isCustomSlab ? (
                    <span className="text-[10px] font-black text-amber-800 dark:text-amber-200 bg-amber-100 dark:bg-amber-950/70 border border-amber-300 dark:border-amber-800 px-2 py-0.5 rounded-full uppercase tracking-wide">
                      ⭐ Admin Slab Rate
                    </span>
                  ) : null}
                  {product.discount && (
                ...
                {/* Interactive Custom Weight Selector - Strictly for kg products */}
                {isKg && (
                  <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-3.5 shadow-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-xs font-black text-[var(--fg)]">
                      <Scale className="h-4 w-4 text-[var(--primary)]" />
                      <span>Choose Weight / Quantity</span>
                    </div>
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
                    {WEIGHT_PRESETS.map((preset) => {
                      const isSelected = !isCustomInputOpen && selectedWeightGrams === preset.grams;
                      return (
                        <button
                          key={preset.grams}
                          type="button"
                          onClick={() => handleSelectPreset(preset.grams)}
                          className={`rounded-xl py-1.5 px-2 text-xs font-extrabold border transition-all text-center ${
                            isSelected
                              ? 'bg-[var(--primary)] text-white border-[var(--primary)] shadow-sm scale-105'
                              : 'bg-[var(--muted)]/40 border-[var(--border)] text-[var(--fg)] hover:border-[var(--primary)]'
                          }`}
                        >
                          {preset.label}
                        </button>
                    ...
                    <button
                      ...
                      className={`rounded-xl py-1.5 px-2 text-xs font-extrabold border transition-all text-center flex items-center justify-center gap-1 col-span-2 sm:col-span-1 ${
                        isCustomInputOpen
                          ? 'bg-[var(--primary)] text-white border-[var(--primary)] shadow-sm'
                          : 'bg-[var(--muted)]/40 border-[var(--border)] text-[var(--primary)] hover:border-[var(--primary)]'
                      }`}
                    >
                  ...
                  {/* Custom Weight Entry Box */}
                  {isCustomInputOpen && (
                    <motion.div
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: 'auto' }}
                      exit={{ opacity: 0, height: 0 }}
                      className="pt-2 border-t border-[var(--border)] space-y-2.5"
                    >
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="font-extrabold text-[var(--fg)]">Enter Any Custom Weight:</span>
                        {/* Unit Switcher */}
                        <div className="flex items-center gap-1 bg-[var(--muted)] p-0.5 rounded-lg">
                          <button
                            type="button"
                            onClick={() => handleUnitToggle('g')}
                            className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold transition-all ${
                              unitMode === 'g' ? 'bg-[var(--card)] text-[var(--primary)] shadow-sm' : 'text-[var(--muted-fg)]'
                            }`}
                          >
                            Grams (g)
                          </button>
                          <button
                            ...
                            className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold transition-all ${
                              unitMode === 'kg' ? 'bg-[var(--card)] text-[var(--primary)] shadow-sm' : 'text-[var(--muted-fg)]'
                            }`}
                          >
                            Kilograms (kg)
                          </button>
                      ...
                      <div className="relative">
                        <input
                          ...
                          placeholder={unitMode === 'kg' ? 'e.g. 1.25kg or 650g' : 'e.g. 300g, 650g, 750g'}
                          className="w-full rounded-xl border border-[var(--border)] bg-[var(--muted)]/40 px-3.5 py-2 text-sm font-bold text-[var(--fg)] placeholder-[var(--muted-fg)]/60 focus:border-[var(--primary)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)]"
                        />
                        <div className="absolute right-3 top-2 text-xs font-black text-[var(--primary)] pointer-events-none">
                          ₹{activePrice}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 pt-1">
                        <span className="text-[10px] text-[var(--muted-fg)] font-bold">Quick Examples:</span>
                        {POPULAR_CUSTOM_SUGGESTIONS.map((sug) => (
                          <button
                            key={sug.label}
                            type="button"
                            onClick={() => {
                              setWeightInput(sug.label);
                              setUnitMode(sug.grams >= 1000 ? 'kg' : 'g');
                            }}
                            className="text-[10px] font-extrabold px-2 py-0.5 rounded-lg bg-[var(--muted)] hover:bg-[var(--primary)] hover:text-white text-[var(--fg)] border border-[var(--border)] transition-all"
                          >
                            {sug.label}
                          </button>
                        ))}
                      ...
                      {/* Grams Conversion & Price Notification */}
                      {priceResult && (
                        <div className="rounded-xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200/60 dark:border-emerald-900/60 p-2.5 text-[11px] text-emerald-800 dark:text-emerald-300">
                          <div className="flex items-center justify-between font-extrabold">
                            <span className="flex items-center gap-1">
                              <Scale className="h-3.5 w-3.5 text-emerald-600" />
                              Selected: {activeGrams.toLocaleString('en-IN')}g
                            </span>
                ...
                <div className="flex items-center gap-3">
                  {quantityInCart > 0 ? (
                      ...
                      onClick={() => {
                        if (isKg) {
                          onAddToCart(product.id, {
                            weight: activeDisplayWeight,
                            weightInGrams: activeGrams,
                            price: activePrice,
                            qty: 1,
                          });
                        } else {
                    ...
                    >
                      ...
                      <span>{oos ? 'SOLD OUT' : (isKg ? `ADD ${activeDisplayWeight} • ₹${activePrice}` : `ADD TO CART • ₹${product.sp}`)}</span>
                    </button>
                  )}
                </div>
              </div>
