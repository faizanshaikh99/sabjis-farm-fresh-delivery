...
import { 
  CheckCircle2, Truck, Printer, 
  ArrowRight, Download, Loader2, User, Phone
} from 'lucide-react';
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg)] p-3 space-y-2">
                {(order.items || []).map((item: any, idx) => {
                  ...
                  const unitPrice = item.price !== undefined ? item.price : (item.sp || 0);
                  const qty = item.quantity !== undefined ? item.quantity : (item.qty || 1);
                  const itemTotal = item.itemTotal !== undefined ? item.itemTotal : (item.lineTotal !== undefined ? item.lineTotal : (unitPrice * qty));

                  return (
                    <div key={idx} className="flex justify-between items-center text-xs gap-2 py-1 border-b border-[var(--border)]/40 last:border-0">
                      <div className="flex items-center gap-1.5 min-w-0 truncate">
                        <span className="text-sm shrink-0">{item.emoji || '🥬'}</span>
                        <span className="font-extrabold text-[var(--fg)] truncate">{displayName}</span>
                        {weight && (
                          <span className="text-[10px] font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.5 rounded border border-emerald-200/50 dark:border-emerald-800/40 shrink-0">
                            {weight}
                          </span>
                        )}
                      ...
                      <span className="font-mono font-black text-[var(--fg)] shrink-0">₹{itemTotal}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="border-t border-[var(--border)] pt-3 space-y-1 text-xs">
              ...
              {order.discountApplied && order.discountApplied > 0 ? (
                <div className="flex justify-between text-emerald-600 font-bold">
                  <span>Coupon Discount ({order.couponApplied || 'SAVINGS'})</span>
                  <span className="font-mono">- ₹{order.discountApplied}</span>
                </div>
              ) : null}
              {order.paymentDiscountAmount && order.paymentDiscountAmount > 0 ? (
                <div className="flex justify-between text-emerald-600 font-bold">
                  <span>Payment Discount ({order.paymentDiscountLabel || order.payment || 'Special'})</span>
                  <span className="font-mono">- ₹{order.paymentDiscountAmount}</span>
                </div>
              ) : null}
              <div className="flex justify-between text-[var(--muted-fg)]">
                <span>Delivery</span>
