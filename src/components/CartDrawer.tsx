...
import { ShoppingBag, X, Minus, Plus, Trash2, Tag, Sparkles, Heart, CheckCircle2, Scale } from 'lucide-react';
...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  // Authoritative Server-side Pricing Verification
  ...
  const [cartVerifyError, setCartVerifyError] = useState<string | null>(null);

  useEffect(() => {
    if (!isOpen || items.length === 0) {
      setVerifiedCart(null);
      setCartVerifyError(null);
      return;
    }
    ...
    fetch('/api/cart/verify', {
      ...
      .then(data => {
        if (data && data.valid) {
          ...
          setCartVerifyError(data?.error || null);
          setVerifiedCart(null);
        }
      })
      .catch(() => {});
  }, [isOpen, items.length, appliedCoupon, JSON.stringify(items.map(i => `${i.id}-${i.qty}-${i.weight}`))]);

  const cartAuthoritativeSubtotal = verifiedCart ? verifiedCart.subtotal : subtotal;
  const cartAuthoritativeDelivery = verifiedCart ? verifiedCart.delivery : finalDelivery;
  const cartAuthoritativeDiscount = verifiedCart ? verifiedCart.couponDiscount : couponDiscount;
  const cartAuthoritativeTotal = verifiedCart ? verifiedCart.total : total;
  ...
  const handleSaveForLater = (item: CartItem) => {
    ...
    if (!savedForLater.some(x => (x.cartKey || x.id) === itemKey)) {
  ...
  const handleMoveToCart = (item: CartItem) => {
    ...
    if (onAddToCart) {
      onAddToCart(item.id, {
        weight: item.weight,
        weightInGrams: item.weightInGrams || 1000,
        price: item.sp,
        qty: item.qty || 1
      });
    }
    ...
    const itemKey = item.cartKey || item.id;
    onUpdateSavedForLater(savedForLater.filter(x => (x.cartKey || x.id) !== itemKey));
  };

  const handleRemoveSavedItem = (id: number | string) => {
    onUpdateSavedForLater(savedForLater.filter(x => (x.cartKey || x.id) !== id));
  };
              ...
              {items.length === 0 ? (
                ...
                <div className="space-y-3">
                  {items.map((item) => {
                    ...
                    const itemKey = item.cartKey || item.id;
                    const verifiedCartItem = verifiedCart?.items?.find(vi => vi.id === item.id);
                    return (
                      <motion.div
                        key={itemKey}
                        layout
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                          <span className="text-[10px] font-extrabold text-[var(--fg)] bg-[var(--muted)] px-1.5 py-0.5 rounded flex items-center gap-1">
                            <Scale className="h-2.5 w-2.5 text-[var(--primary)]" />
                            {item.weight}
                          </span>
                          {item.weightInGrams && (
                            <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400">
                              ({item.weightInGrams.toLocaleString('en-IN')}g)
                            </span>
                          )}
                        </div>
                        <div className="flex items-baseline gap-1 mt-1.5">
                          <p className="text-xs font-extrabold text-[var(--primary)]">
                            ₹{verifiedCartItem ? verifiedCartItem.lineTotal : (item.sp * item.qty)}
                          </p>
                          <span className="text-[9px] text-[var(--muted-fg)]">
                            (₹{verifiedCartItem ? verifiedCartItem.sp : item.sp} / {item.weight})
                          </span>
                        </div>
                        <button
              ...
              <div className="border-t border-[var(--border)] bg-[var(--card)] p-4 space-y-3.5">
                ...
                <div className="flex items-center justify-between text-[11px] text-emerald-700 dark:text-emerald-400 font-bold">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>Authoritative Pricing Verified</span>
                  </span>
                </div>

                {cartVerifyError && (
                  <div className="p-2.5 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl text-xs text-red-700 dark:text-red-300 font-medium">
                    ⚠️ {cartVerifyError}
                  </div>
                )}
                ...
                <div className="space-y-1.5 text-xs text-[var(--muted-fg)]">
                  <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span className="text-[var(--fg)] font-semibold">₹{cartAuthoritativeSubtotal}</span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="text-[var(--fg)] font-semibold">
                      {isFreeDeliveryApplied ? (
                        <span className="text-emerald-600 font-extrabold flex items-center gap-1">
                          <span className="line-through text-[var(--muted-fg)] font-normal">₹{cartAuthoritativeDelivery}</span>
                          <span>FREE (COUPON)</span>
                        </span>
                      ) : cartAuthoritativeDelivery === 0 ? (
                        <span className="text-emerald-600 font-bold">FREE</span>
                      ) : (
                        `₹${cartAuthoritativeDelivery}`
                      )}
                    </span>
                  </div>

                  {cartAuthoritativeDiscount > 0 && (
                    <div className="flex justify-between text-emerald-600 font-bold">
                      <span>Coupon Discount ({appliedCoupon})</span>
                      <span>- ₹{cartAuthoritativeDiscount}</span>
                    </div>
                  )}

                  {cartAuthoritativeSubtotal < 299 && !isFreeDeliveryApplied && (
                    <p className="text-[10px] bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 rounded-lg p-2 font-medium">
                      🚚 Add <strong>₹{299 - cartAuthoritativeSubtotal}</strong> more to qualify for <strong>FREE Delivery</strong>, or apply <strong>FREE90</strong>!
                    </p>
                  )}

                  <div className="flex justify-between text-sm font-extrabold text-[var(--fg)] pt-2 border-t border-[var(--border)] mt-2">
                    <span>Cart Total</span>
                    <span className="text-[var(--primary)] text-base font-black">₹{cartAuthoritativeTotal}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <button
                    onClick={onCheckout}
                    disabled={Boolean(cartVerifyError)}
                    className="w-full bg-[var(--primary)] text-white text-xs font-extrabold rounded-full py-3 hover:opacity-95 shadow-md active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
                    Proceed to Checkout · ₹{cartAuthoritativeTotal}
                  </button>
                  <button
                    onClick={onClearCart}
