...
import { User, CartItem, Address, PaymentSettings, BusinessSettings, PaymentMethodDiscount } from '../types';
import { MapPin, CreditCard, ShoppingCart, Plus, X, Tag, FileText, CheckCircle2, ShieldCheck, Check, AlertTriangle, AlertCircle, Loader2, Sparkles, Clock, RefreshCw, Smartphone } from 'lucide-react';
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  // Payment Method Discounts State & Fetching
  const [paymentDiscounts, setPaymentDiscounts] = useState<PaymentMethodDiscount[]>([]);

  useEffect(() => {
    fetch('/api/payment-method-discounts')
      .then(res => res.ok ? res.json() : null)
      .then(data => {
        if (data && Array.isArray(data.discounts)) {
          setPaymentDiscounts(data.discounts);
        }
      })
      .catch(() => {});
  }, []);
  ...
  // Authoritative Backend Price Verification State
  ...
  // Call backend verification engine to ensure prices, weights, slabs, and stocks are authentic
  useEffect(() => {
    if (cartItems.length === 0) return;
    setIsVerifyingPrice(true);
    setVerificationError(null);

    fetch('/api/cart/verify', {
      method: 'POST',
      ...
      .then(data => {
        ...
        if (data && data.valid) {
          setVerifiedPricing(data);
          setVerificationError(null);
        } else {
          setVerificationError(data?.error || 'Pricing verification failed for items in your cart.');
          setVerifiedPricing(null);
        }
      })
      .catch(err => {
        setIsVerifyingPrice(false);
        console.error('Cart price verification error:', err);
      });
  }, [cart, appliedCoupon, selectedPayment, user?.id]);

  // Client-side helper to compute discount information for each option
  const getDiscountForOption = (optionId: string) => {
    if (!paymentDiscounts.length) return null;
    const normKey = (optionId || '').toLowerCase();
    const config = paymentDiscounts.find(d => {
      if ((d.status || 'active').toLowerCase() !== 'active') return false;
      const dKey = (d.paymentMethod || '').toLowerCase();
      if (dKey === normKey) return true;
      if (normKey === 'gpay' && (dKey === 'google pay' || dKey === 'gpay')) return true;
      if (normKey === 'cod' && (dKey === 'cash on delivery' || dKey === 'cod')) return true;
      if (normKey === 'upi' && (dKey === 'upi' || dKey === 'qr')) return true;
      return false;
    });
    ...
    const currentSubtotal = verifiedPricing ? verifiedPricing.subtotal : subtotal;
    const minOrderMet = !config.minOrder || currentSubtotal >= config.minOrder;
    const shortfall = config.minOrder && currentSubtotal < config.minOrder ? config.minOrder - currentSubtotal : 0;

    let discountAmount = 0;
    if (config.discountType === 'percent') {
      discountAmount = Math.round((currentSubtotal * config.discountValue) / 100);
      if (config.maxDiscount && config.maxDiscount > 0) {
        discountAmount = Math.min(discountAmount, config.maxDiscount);
      }
    } else {
      discountAmount = Math.min(config.discountValue, currentSubtotal);
    }
  ...
  const selectedDiscount = getDiscountForOption(selectedPayment);

  // Authoritative figures derived from backend verification
  const authoritativeSubtotal = verifiedPricing ? verifiedPricing.subtotal : subtotal;
  const authoritativeDelivery = verifiedPricing ? verifiedPricing.delivery : finalDelivery;
  const authoritativeDiscount = verifiedPricing ? verifiedPricing.couponDiscount : couponDiscount;
  const authoritativePaymentDiscountAmount = verifiedPricing?.paymentDiscountAmount !== undefined
    ? verifiedPricing.paymentDiscountAmount
    : (selectedDiscount && selectedDiscount.minOrderMet ? selectedDiscount.discountAmount : 0);
  const authoritativeTotal = verifiedPricing 
    ? verifiedPricing.total 
    : Math.max(0, authoritativeSubtotal + authoritativeDelivery - authoritativeDiscount - authoritativePaymentDiscountAmount);
  const totalSavings = authoritativeDiscount + authoritativePaymentDiscountAmount;
  ...
  const finalPaymentOptions = paymentOptions.map(opt => {
    const discountInfo = getDiscountForOption(opt.id);
    return {
      ...opt,
      discountInfo,
      buttonLabel: opt.id === 'cod'
        ? `Place COD Order • ₹${authoritativeTotal}`
        : `Pay ₹${authoritativeTotal} with ${opt.title}`
    };
  });
  ...
  const handleConfirm = () => {
    ...
    if (verificationError) {
      setErrorMsg(`Authoritative verification error: ${verificationError}`);
      return;
    }
    if (isVerifyingPrice) {
      setErrorMsg('Please wait a moment while the backend verifies item prices and stock.');
      return;
    }
    setErrorMsg(null);
    const selectedAddress = addresses[selectedAddressIdx] || addresses[0];
      ...
      >
        ...
        >
          ...
          <div className="space-y-3">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 pt-1.5 border-t border-[var(--border)]/40 flex items-center justify-between gap-1 text-[9px]">
                      {opt.discountInfo ? (
                        opt.discountInfo.minOrderMet ? (
                          <span className="px-1.5 py-0.5 rounded font-black text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950 border border-emerald-300 dark:border-emerald-800 flex items-center gap-1">
                            <span>🎉</span>
                            <span>Save ₹{opt.discountInfo.discountAmount} ({opt.discountInfo.label})</span>
                          </span>
                        ) : (
                          <span className="px-1.5 py-0.5 rounded font-bold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800">
                            Add ₹{opt.discountInfo.shortfall} for {opt.discountInfo.label}
                          </span>
                        )
                      ) : (
            ...
            {/* Payment Method Discount Live Callout */}
            {selectedDiscount && selectedDiscount.minOrderMet && (
              <div className="mt-2.5 p-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 flex items-center justify-between text-xs text-emerald-900 dark:text-emerald-200">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-base shrink-0">🎉</span>
                  <div className="min-w-0">
                    <span className="font-extrabold block truncate">
                      {selectedDiscount.config.paymentMethodName || selectedOption.title} Discount Applied!
                    </span>
                    <span className="text-[10px] text-emerald-700 dark:text-emerald-400 block">
                      You save ₹{authoritativePaymentDiscountAmount || selectedDiscount.discountAmount} with {selectedOption.title}.
                      {selectedDiscount.config.discountType === 'percent' && selectedDiscount.config.maxDiscount ? ` (Capped at ₹${selectedDiscount.config.maxDiscount})` : ''}
                    </span>
                  </div>
                </div>
                <span className="font-black text-xs text-emerald-700 dark:text-emerald-300 bg-white dark:bg-emerald-900/60 px-2 py-1 rounded-lg border border-emerald-300 dark:border-emerald-700 shrink-0 ml-2">
                  -₹{authoritativePaymentDiscountAmount || selectedDiscount.discountAmount}
                </span>
              </div>
            )}

            {selectedDiscount && !selectedDiscount.minOrderMet && (
              <div className="mt-2.5 p-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 flex items-center justify-between text-xs text-amber-900 dark:text-amber-200">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-base shrink-0">💡</span>
                  <div className="min-w-0">
                    <span className="font-extrabold block truncate">
                      Pay with {selectedOption.title} & Save {selectedDiscount.label}
                    </span>
                    <span className="text-[10px] text-amber-700 dark:text-amber-400 block">
                      Add ₹{selectedDiscount.shortfall} more to qualify (Min. order ₹{selectedDiscount.config.minOrder}).
                    </span>
                  </div>
                </div>
                <span className="font-black text-[10px] text-amber-800 dark:text-amber-300 bg-white dark:bg-amber-900/60 px-2.5 py-1 rounded-lg border border-amber-300 dark:border-amber-700 shrink-0 ml-2">
                  Min. ₹{selectedDiscount.config.minOrder}
                </span>
              </div>
            )}
          </div>
          ...
          {/* Section 4: Summary of Checkout Items */}
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-[var(--fg)] uppercase tracking-wider flex items-center gap-1">
              <span>🛒</span>
              <span>Order Summary</span>
            </h3>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)] p-4 space-y-2.5 max-h-[160px] overflow-y-auto">
              {cartItems.map((item) => {
                const verifiedItem = verifiedPricing?.items?.find(vi => vi.id === item.id);
                const displayUnitSp = verifiedItem ? verifiedItem.sp : item.sp;
                const displayLineTotal = verifiedItem ? verifiedItem.lineTotal : (item.sp * item.qty);
                const displayWeight = verifiedItem ? verifiedItem.weight : item.weight;

                return (
                  <div key={item.id} className="flex justify-between items-start text-xs border-b border-[var(--border)]/50 pb-2 last:border-b-0 last:pb-0">
                    <div className="flex-1 pr-2">
                      <div className="text-[var(--fg)] font-medium flex items-center gap-1.5">
                        <span>{item.emoji} {item.name}</span>
                        <strong className="text-[var(--muted-fg)] font-bold">× {item.qty}</strong>
                      </div>
                      <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[10px] text-[var(--muted-fg)]">
                        <span className="bg-[var(--muted)] px-1.5 py-0.2 rounded font-semibold text-[var(--fg)]">
                          {displayWeight}
                        </span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <span className="font-bold text-[var(--fg)] text-xs">₹{displayLineTotal}</span>
                      <p className="text-[9px] text-[var(--muted-fg)]">₹{displayUnitSp} each</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="p-5 border-t border-[var(--border)] bg-[var(--card)] flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between text-[11px] px-1">
            <span className="flex items-center gap-1.5 font-bold text-emerald-700 dark:text-emerald-400">
              ...
              <span>Backend Authoritative Pricing Verified</span>
            </span>
            {isVerifyingPrice && (
              <span className="text-[10px] text-[var(--muted-fg)] font-semibold animate-pulse">
                Recalculating...
              </span>
            )}
          </div>
          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between text-[var(--muted-fg)]">
              <span>Subtotal</span>
              <span className="font-semibold text-[var(--fg)]">₹{authoritativeSubtotal}</span>
            </div>
            <div className="flex justify-between text-[var(--muted-fg)]">
              ...
              <span>
                ...
                ) : authoritativeDelivery === 0 ? (
                  <span className="text-emerald-600 font-bold">FREE</span>
                ) : (
                  <span className="font-semibold text-[var(--fg)]">₹{authoritativeDelivery}</span>
                )}
              </span>
            </div>
            ...
            {authoritativeDiscount > 0 && (
              <div className="flex justify-between text-emerald-600 font-bold">
                ...
                <span>- ₹{authoritativeDiscount}</span>
              </div>
            )}

            {authoritativePaymentDiscountAmount > 0 && (
              <div className="flex justify-between text-emerald-600 dark:text-emerald-400 font-bold bg-emerald-50 dark:bg-emerald-950/40 px-2 py-1 rounded-lg border border-emerald-200 dark:border-emerald-800">
                <span className="flex items-center gap-1 truncate">
                  <span>🏷️</span>
                  <span className="truncate">
                    {verifiedPricing?.paymentDiscount?.methodName || selectedOption.title} (
                    {verifiedPricing?.paymentDiscount?.label || selectedDiscount?.label || 'Special Discount'}
                    )
                  </span>
                </span>
                <span className="shrink-0 ml-2">- ₹{authoritativePaymentDiscountAmount}</span>
              </div>
            )}

            <div className="flex justify-between text-base font-extrabold text-[var(--fg)] pt-1.5 border-t border-[var(--border)] mt-1.5">
              <span>Checkout Total</span>
              <span className="text-[var(--primary)] text-lg font-black">₹{authoritativeTotal}</span>
            </div>

            {totalSavings > 0 && (
              <div className="text-[11px] font-extrabold text-emerald-700 dark:text-emerald-300 text-right">
          ...
          {verificationError && (
            <div className="p-3 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl flex items-start gap-2 text-xs text-red-700 dark:text-red-300 font-semibold">
              <AlertCircle className="h-4 w-4 text-red-600 flex-shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="font-bold">Authoritative Price Verification Notice</p>
          ...
          <button
            ...
            disabled={!addresses.length || isProcessingPayment || Boolean(verificationError) || isVerifyingPrice}
          ...
          >
            {isProcessingPayment ? (
              ...
              <span>Verifying Pricing...</span>
            ) : (
              <span>{selectedOption.buttonLabel} (₹{authoritativeTotal}) →</span>
            )}
          </button>
        </div>
