...
export const generateInvoicePdf = async (
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (item.weight) {
      cleanName = `${cleanName} (${item.weight})`;
    }
  ...
  if (order.discountApplied && order.discountApplied > 0) {
    doc.text(`Coupon Discount (${order.couponApplied || 'SAVINGS'}):`, labelAlignX, y);
    doc.text(`- Rs. ${order.discountApplied}`, rightAlignX, y, { align: 'right' });
    y += 4.5;
  }

  if (order.paymentDiscountAmount && order.paymentDiscountAmount > 0) {
    doc.text(`Payment Discount (${order.paymentDiscountLabel || order.payment || 'Special'}):`, labelAlignX, y);
    doc.text(`- Rs. ${order.paymentDiscountAmount}`, rightAlignX, y, { align: 'right' });
    y += 4.5;
  }
