...
import { supabaseClient, AppRealtimeHub } from './lib/supabaseClient';
...
import { getBaseWeightInGrams, calculateWeightPrice, isKgProduct } from './utils/weightParser';
import { sortOrdersNewestFirst, normalizeOrderStatus } from './utils/orderStatus';
import {
  Heart,
  ShoppingBag,
  Search,
...
export default function App() {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_products');
      if (stored) {
  ...
  const [categories, setCategories] = useState<Category[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_categories');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
  ...
  const [offers, setOffers] = useState<Offer[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_offers');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
  ...
  const [reviews, setReviews] = useState<Review[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_reviews');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
  ...
  const [users, setUsers] = useState<User[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_users');
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    try {
      const stored = localStorage.getItem('sabjies_current_user');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && parsed.id && parsed.id !== 'u_pree') return parsed;
      }
      return null;
    } catch {
      return null;
  ...
  // Cart & Wishlist & RV
  const [cart, setCart] = useState<{ [id: string | number]: CartItem }>(() => {
    try {
      const stored = localStorage.getItem('sabjies_cart');
      return stored ? JSON.parse(stored) : {};
    } catch {
      return {};
    }
  ...
  const [wishlist, setWishlist] = useState<number[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_wishlist');
      return stored ? JSON.parse(stored) : [];
    } catch {
      return [];
    }
  ...
  const [recentlyViewed, setRecentlyViewed] = useState<number[]>(() => {
    try {
      const stored = localStorage.getItem('sabjies_rv');
      return stored ? JSON.parse(stored) : [];
    } catch {
      return [];
    }
  ...
  const [profileInitialTab, setProfileInitialTab] = useState<'profile' | 'orders' | 'addresses' | 'wishlist' | 'payment_history' | 'settings'>('orders');
  ...
  // ⚡ CONTINUOUS REAL-TIME ORDER SYNCHRONIZATION (Admin ↔ Customer)
  useEffect(() => {
        ...
        .then((data) => {
          if (Array.isArray(data)) {
            setOrders((prevOrders) => {
              if (prevOrders.length > 0) {
                data.forEach((newOrder) => {
                  ...
                  if (oldOrder) {
                    ...
                    const isRelevantOrder = currentUser
                      ? (newOrder.userId === currentUser.id || (currentUser.email && newOrder.userEmail === currentUser.email))
                      : true;
              ...
              return sortOrdersNewestFirst(data);
            });
          }
        })
        .catch(() => {});
    ...
    // Subscribe to live order changes from server/admin
    const channel = supabaseClient
      ...
      .on(
        ...
        (payload: any) => {
          console.log('⚡ [App Realtime] Order event received:', payload);
          if (payload?.new) {
            const updated = payload.new;
            setOrders((prevOrders) => {
              const oldOrder = prevOrders.find((o) => String(o.id) === String(updated.id));
              if (oldOrder && oldOrder.status !== updated.status) {
                const isRelevant = currentUser
                  ? (updated.userId === currentUser.id || (currentUser.email && updated.userEmail === currentUser.email))
                  : true;
                if (isRelevant) {
                  showToast(
                    `📦 Order #${updated.id} status updated to ${updated.status.toUpperCase()}!`,
                    'info'
                  );
                }
              }
              const exists = prevOrders.some((o) => String(o.id) === String(updated.id));
              const nextList = exists
                ? prevOrders.map((o) => (String(o.id) === String(updated.id) ? { ...o, ...updated } : o))
                : [updated, ...prevOrders];
              return sortOrdersNewestFirst(nextList);
            });
          }
          fetchLatestOrders();
        }
    ...
    // Resilient fallback sync interval (every 20s, paused when tab is inactive)
  ...
  // Notifications sync for logged in user (every 30s when tab is active)
  useEffect(() => {
    if (!currentUser) {
      setNotifications([]);
      return;
  ...
  const pendingProductIdRef = React.useRef<number | string | null>(null);

  // Helper to parse the current URL and apply the corresponding view state
  const parseCurrentRoute = (availableProducts: Product[] = products) => {
    ...
    // 2. Orders route: /orders or /my-orders
    ...
    // 4. Addresses route: /addresses
    if (pathname === '/addresses') {
      setProfileInitialTab('addresses');
      setIsProfileOpen(true);
      return;
    }
    ...
    // 7. Wishlist route: /wishlist
    if (pathname === '/wishlist') {
      setIsWishlistOpen(true);
      return;
    }

    // 8. Product route: /product/:id or /#product-:id or ?product=:id
    let targetId: string | null = null;
    if (pathname.startsWith('/product/')) {
      const raw = pathname.replace('/product/', '').split('/')[0];
      if (raw) targetId = raw;
    } else if (hash.startsWith('#product-')) {
      const raw = hash.replace('#product-', '');
      if (raw) targetId = raw;
    } else if (searchParams.get('product')) {
      targetId = searchParams.get('product');
    }

    if (targetId) {
      pendingProductIdRef.current = targetId;
      const found = availableProducts.find((p) => String(p.id) === String(targetId));
      if (found) {
    ...
    // 9. Category route: /category/:cat or ?category=:cat
  ...
  const initData = async () => {
    ...
    // Guaranteed failsafe: Maximum 1.0 second loading ceiling under any condition
    const hardCeilingTimer = setTimeout(() => {
      setAppLoading(false);
    }, 1000);

    const fetchWithTimeout = async <T,>(url: string, fallback: T, timeoutMs = 800): Promise<T> => {
    ...
    try {
      ...
      // 2. Synchronously load Client Preferences & Local Cart/Wishlist/RV
      try {
      ...
      // 3. Prioritize Essential Content: Fetch products and categories concurrently
      const [productsRes, categoriesRes] = await Promise.all([
        fetchWithTimeout('/api/products', INITIAL_PRODUCTS, 800),
        fetchWithTimeout('/api/categories', CATEGORIES, 800)
      ]);

      const resolvedProducts = Array.isArray(productsRes) && productsRes.length > 0 ? productsRes : INITIAL_PRODUCTS;
      setProducts(resolvedProducts);
      try {
        localStorage.setItem('sabjies_products', JSON.stringify(resolvedProducts));
      } catch {}

      if (pendingProductIdRef.current) {
        const found = resolvedProducts.find((p: Product) => String(p.id) === String(pendingProductIdRef.current));
        if (found) {
          setSelectedDetailedProduct(found);
          setIsProductDetailsOpen(true);
          pendingProductIdRef.current = null;
      ...
      if (Array.isArray(categoriesRes) && categoriesRes.length > 0) {
        ...
        try {
          localStorage.setItem('sabjies_categories', JSON.stringify(categoriesRes));
        } catch {}
      }

      // 4. ESSENTIAL CONTENT IS READY: UNBLOCK UI IMMEDIATELY (Zero artificial delay)
      clearTimeout(hardCeilingTimer);
      setAppLoading(false);

      // 5. Hydrate secondary entities non-blockingly in the background
      fetchWithTimeout('/api/offers', INITIAL_OFFERS, 2500)
        .then(res => {
          if (Array.isArray(res) && res.length > 0) {
            setOffers(res);
            try { localStorage.setItem('sabjies_offers', JSON.stringify(res)); } catch {}
          }
        })
        .catch(() => {});

      fetchWithTimeout('/api/reviews', INITIAL_REVIEWS, 2500)
        .then(res => {
          if (Array.isArray(res) && res.length > 0) {
            setReviews(res);
            try { localStorage.setItem('sabjies_reviews', JSON.stringify(res)); } catch {}
          }
        })
        .catch(() => {});

      fetchWithTimeout('/api/orders', [], 2500)
        .then(res => {
          if (Array.isArray(res)) setOrders(sortOrdersNewestFirst(res));
        })
        .catch(() => {});

      // 6. Load Users from cache
      ...
      // 7. Current User Session & live profile/addresses refresh
      ...
      // 8. Background Business Settings
      fetchWithTimeout('/api/business-settings', null, 2500)
        .then(data => {
          if (data) {
            setBusinessSettings({
  ...
  // Route parser on initial mount and browser history navigation (popstate)
  useEffect(() => {
    parseCurrentRoute();
  ...
  // Sync state to URL pathname without page reloads
  useEffect(() => {
    ...
    if (isAdminOpen) {
      ...
      const target = profileInitialTab === 'orders' ? '/orders' : '/profile';
      if (currentPath !== target && currentPath !== '/orders' && currentPath !== '/my-orders' && currentPath !== '/profile') {
        window.history.pushState({ view: 'profile' }, '', target);
      }
    } else if (isCartOpen) {
      if (currentPath !== '/cart') {
      ...
      if (currentPath !== '/wishlist') {
        window.history.pushState({ view: 'wishlist' }, '', '/wishlist');
      }
    } else if (isProductDetailsOpen && selectedDetailedProduct) {
      const target = `/product/${selectedDetailedProduct.id}`;
      if (currentPath !== target) {
        window.history.pushState({ view: 'product', id: selectedDetailedProduct.id }, '', target);
      }
    } else {
      const knownSubroutes = ['/admin', '/orders', '/my-orders', '/profile', '/addresses', '/cart', '/checkout', '/wishlist'];
      if (knownSubroutes.includes(currentPath) || currentPath.startsWith('/product/')) {
        window.history.pushState({}, '', '/');
      }
    }
  }, [isAdminOpen, isProfileOpen, profileInitialTab, isCheckoutOpen, isCartOpen, isWishlistOpen, isProductDetailsOpen, selectedDetailedProduct, appLoading]);
  ...
  const handleAddToCart = (
    ...
    const newCart = { ...cart };
    const cartKey = isKg && customWeight ? `${id}_${customWeight.weightInGrams}g` : String(id);
    const addedQty = customWeight?.qty || 1;
    const finalWeight = isKg ? (customWeight?.weight || p.weight) : (p.weight || `per ${p.unit || 'piece'}`);
    const weightInGrams = isKg ? (customWeight?.weightInGrams || getBaseWeightInGrams(p.weight)) : undefined;
    const autoCalc = isKg ? calculateWeightPrice(p.sp, p.weight, weightInGrams || 1000, p.weightSlabs) : null;
    const finalSp = isKg ? (customWeight?.price !== undefined ? customWeight.price : (autoCalc ? autoCalc.price : p.sp)) : p.sp;

    if (newCart[cartKey]) {
      newCart[cartKey].qty += addedQty;
    } else {
      newCart[cartKey] = {
        id: p.id,
        cartKey,
    ...
    showToast(`Added ${p.name} (${finalWeight}) to cart!`, 'success');
  };
  ...
  const getProductCartQty = useCallback((productId: number) => {
    return productCartQtyMap[productId] || 0;
  }, [productCartQtyMap]);
  ...
  const handleUpdateQuantity = (id: number | string, delta: number) => {
    ...
    const key = String(id);
    let targetKey = key;
    if (!newCart[targetKey]) {
      const numId = typeof id === 'number' ? id : parseInt(key, 10);
      const foundKey = Object.keys(newCart).find((k) => newCart[k].id === numId);
      if (foundKey) targetKey = foundKey;
      else return;
    }

    newCart[targetKey].qty += delta;
    if (newCart[targetKey].qty <= 0) {
  ...
  const handleConfirmCheckout = async (
    ...
    try {
      // POST order to backend. The backend strictly and authoritatively verifies:
      // - Product exists
      // - Product is available
      // - Weight is valid
      // - Custom weight is valid
      ...
      if (!response.ok || !responseData.id) {
        showToast(responseData.error || 'Order placement failed server price or stock verification.', 'error');
        return;
      }

      const verifiedOrder: Order = responseData;

      // Update inventory stock levels
      const updatedProducts = products.map((p) => {
        const itemsForProduct = cartItems.filter((item) => item.id === p.id);
        const totalSoldQty = itemsForProduct.reduce((acc, item) => acc + item.qty, 0);
        if (totalSoldQty > 0) {
          return { ...p, stockQty: Math.max(0, p.stockQty - totalSoldQty) };
        }
        return p;
      });
      ...
      const updatedOrders = sortOrdersNewestFirst([verifiedOrder, ...orders.filter(o => o.id !== verifiedOrder.id)]);
      ...
      showToast(`🎉 Order #${verifiedOrder.id} placed successfully! Total: ₹${verifiedOrder.total}`, 'success');
    } catch (err: any) {
      console.error('Error placing verified order:', err);
      showToast(err.message || 'An error occurred while communicating with the order verification server.', 'error');
    }
  };
  ...
  const handleUpdateOrderStatus = async (orderId: string, status: any) => {
    const updated = sortOrdersNewestFirst(orders.map((o) => (o.id === orderId ? { ...o, status, updatedAt: new Date().toISOString() } : o)));
    setOrders(updated);
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.order) {
          AppRealtimeHub.getInstance().publish({
            table: 'orders',
            eventType: 'UPDATE',
            new: data.order
          });
  ...
  const handleUpdateOrderPaymentStatus = async (
    ...
    const updated = sortOrdersNewestFirst(orders.map((o) => {
      if (o.id === orderId) {
        return {
          ...o,
          paymentStatus,
          status: (paymentStatus === 'Paid' && normalizeOrderStatus(o.status) === 'New Orders') ? ('Confirmed' as const) : o.status,
  ...
  const handleUpdateProduct = async (id: number, updates: Partial<Product>) => {
    ...
    const isKg = isKgProduct(merged);
    const sanitizedUpdates = { ...updates };
    if (!isKg) {
      sanitizedUpdates.weightSlabs = [];
      sanitizedUpdates.pricingMode = 'auto';
    }
    const updated = products.map((p) => (p.id === id ? { ...p, ...sanitizedUpdates } : p));
    setProducts(updated);
    try {
      await fetch(`/api/products/${id}`, {
        method: 'PATCH',
  ...
  // FILTERED CATALOG SELECTION (Memoized for high scroll & re-render performance)
  const filteredProducts = useMemo(() => {
    const query = searchQuery.toLowerCase().trim();
    return products.filter((p) => {
      const catOk = selectedCat === 'all' || p.cat === selectedCat;
      const tabOk = selectedTab === 'all' || p.type === selectedTab;
      if (!catOk || !tabOk) return false;

      if (!query) return true;
      const category = categories.find((c) => c.id === p.cat);
      const catLabel = category ? category.label.toLowerCase() : '';
  ...
  }, [products, selectedCat, selectedTab, categories, searchQuery]);
  ...
  const cartTotalQty = useMemo(() => {
    return (Object.values(cart) as CartItem[]).reduce((acc, item) => acc + item.qty, 0);
  }, [cart]);

  const cartSubtotal = useMemo(() => {
    return (Object.values(cart) as CartItem[]).reduce((acc, item) => acc + item.sp * item.qty, 0);
  }, [cart]);
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--fg)] selection:bg-[var(--primary)] selection:text-white">
      {/* ── MODERN INITIAL LOADING SCREEN OVERLAY (Max 1s ceiling, auto-dismisses immediately when ready) ── */}
      <ModernLoadingScreen isLoading={appLoading} />
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            <div className="hidden md:flex items-center gap-2.5">
              ...
              <button
                ...
                className="relative flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--card)] text-[var(--muted-fg)] px-3 py-1.5 text-xs font-bold transition-all hover:border-[var(--primary)] hover:text-[var(--primary)] shadow-2xs cursor-pointer"
                title="Customer Order History"
              >
                <Package className="h-4 w-4 text-[var(--primary)]" />
                <span className="hidden sm:inline">Orders</span>
                {currentUser && (
                  <span className="text-[10px] font-black bg-[var(--primary)]/10 text-[var(--primary)] rounded-full px-1.5 py-0.2">
                    {orders.filter((o) => o.userId === currentUser.id || o.userEmail === currentUser.email).length}
                  </span>
                )}
              </button>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        ...
                        <span className="text-[10px] text-[var(--muted-fg)] font-semibold">
                          {currentUser ? `${orders.filter((o) => o.userId === currentUser.id || o.userEmail === currentUser.email).length} Orders` : 'Sign in'}
                        </span>
                      </button>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {filteredProducts.map((prod) => (
            <ProductCard
              key={prod.id}
              product={prod}
              quantityInCart={productCartQtyMap[prod.id] || 0}
      ...
      <AnimatePresence>
        {isProductDetailsOpen && selectedDetailedProduct && (
          <ProductDetailsModal
            ...
            quantityInCart={getProductCartQty(selectedDetailedProduct.id)}
