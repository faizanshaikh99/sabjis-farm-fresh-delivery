...
export function normalizeOrderItem(it: any, dbProduct?: any) {
  ...
  return {
    ...
    img: it.img || (dbProduct ? dbProduct.img : '')
  };
}
...
function scheduleBackgroundSupabaseSync() {
  ...
  migrationDebounceTimer = setTimeout(() => {
...
async function initSupabaseData() {
  ...
  try {
    ...
    if (dbProducts && dbProducts.length > 0) {
      productsStore = dbProducts.map((p: any) => normalizeProduct({
        id: Number(p.id),
        name: p.name,
        cat: p.cat,
        type: p.type,
        cp: Number(p.cp),
        sp: Number(p.sp),
        basePricePerKg: p.base_price_per_kg !== null && p.base_price_per_kg !== undefined
          ? Number(p.base_price_per_kg)
          : (p.basePricePerKg !== undefined ? Number(p.basePricePerKg) : Number(p.sp)),
        weight: p.weight,
        discount: p.discount,
        img: p.img,
        ...
        weightSlabs: p.weight_slabs || p.weightSlabs || [],
        pricingMode: p.pricing_mode || p.pricingMode || 'auto'
      }));
    }
    ...
    if (dbOrders && dbOrders.length > 0) {
      ordersStore = dbOrders.map((o: any) => normalizeOrder({
        id: o.id,
...
function isAdminRequest(req: express.Request): boolean {
  ...
  // 2. User ID header accompanied by active session validation
  if (userId) {
    const user = usersStore.find((u: any) => String(u.id) === String(userId) || (u.email && u.email.toLowerCase() === String(userId).toLowerCase()));
    if (user && isUserAdmin(user)) {
      if (sessionId) {
        const session = sessionsStore.find((s: any) => 
          (s.id === sessionId || s.token === sessionId || String(s.userId) === String(user.id)) &&
          (!s.expiresAt || new Date() <= new Date(s.expiresAt))
        );
        if (session) return true;
      } else {
        const activeSession = sessionsStore.find((s: any) => 
...
async function verifyAdminRequest(req: express.Request): Promise<boolean> {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (userId) {
        let user = usersStore.find((u: any) => String(u.id) === String(userId));
        if (!user) {
          const { data: dbUser } = await supabase
            .from('users')
            .select('*')
        ...
        if (user && isUserAdmin(user)) {
          ...
          if (activeSess && (!activeSess.expires_at || new Date() <= new Date(activeSess.expires_at))) {
            return true;
          }
        }
      }
...
app.get("/api/products", async (req, res) => {
  // If productsStore is already in memory, serve immediately to guarantee sub-5ms response
  if (Array.isArray(productsStore) && productsStore.length > 0) {
    res.json(productsStore.map(normalizeProduct));
    // Refresh cache in background if Supabase is active
    if (isSupabaseConfigured && supabase) {
      (async () => {
        try {
          ...
          if (!error && data && data.length > 0) {
            productsStore = data.map((p: any) => normalizeProduct({
              id: Number(p.id),
              name: p.name,
              cat: p.cat,
              type: p.type,
    ...
    if (isSupabaseConfigured && supabase && !isNaN(Number(id))) {
      ...
      if (!error && data) {
        const prod = normalizeProduct({
          ...
          cp: Number(data.cp),
          sp: Number(data.sp),
          basePricePerKg: data.base_price_per_kg !== null && data.base_price_per_kg !== undefined
            ? Number(data.base_price_per_kg)
            : (data.basePricePerKg !== undefined ? Number(data.basePricePerKg) : Number(data.sp)),
          weight: data.weight,
          ...
          lowAt: Number(data.low_at),
          weightSlabs: data.weight_slabs || data.weightSlabs || [],
          pricingMode: data.pricing_mode || data.pricingMode || 'auto'
        });
...
const handleProductUpdate = async (req: express.Request, res: express.Response) => {
  const id = Number(req.params.id);
  const { stockQty, sp, basePricePerKg, base_price_per_kg, name, cat, type, cp, weight, unit, discount, img, images, emoji, lowAt, weightSlabs, weight_slabs, pricingMode } = req.body;
    ...
    if (p.id === id) {
      ...
      const rawPrice = basePricePerKg !== undefined
        ...
        : (base_price_per_kg !== undefined
            ...
            : (sp !== undefined ? Number(sp) : p.sp));

      const newUnit = unit !== undefined
        ? unit
        : (p.unit || (isKgProduct(p) ? 'kg' : getProductSellingUnit(p)));
      const newWeight = weight !== undefined ? weight : p.weight;
      const isKg = isKgProduct({ unit: newUnit, weight: newWeight });

      // STRICT VALIDATION: If product is changed to a non-kg unit, clear any weight slabs
      const updatedSlabs = isKg
        ? (weightSlabs !== undefined
            ? (Array.isArray(weightSlabs) ? weightSlabs : [])
            : (weight_slabs !== undefined ? (Array.isArray(weight_slabs) ? weight_slabs : []) : p.weightSlabs))
        : [];
      ...
      targetProduct = normalizeProduct({
        ...
        pricingMode: isKg ? (pricingMode !== undefined ? pricingMode : (updatedSlabs && updatedSlabs.length > 0 ? 'slabs' : p.pricingMode)) : 'auto'
      });
  ...
  if (targetProduct && isSupabaseConfigured) {
    await syncProductToSupabase(targetProduct);
  }
...
app.get("/api/orders", async (req, res) => {
    ...
    if (isSupabaseConfigured && supabase) {
      (async () => {
        try {
          ...
          if (!error && data) {
            ordersStore = data.map((o: any) => normalizeOrder({
              id: o.id,
              userId: o.user_id || o.customer_id,
              userEmail: o.user_email,
              userName: o.user_name,
  ...
  try {
    if (isSupabaseConfigured && supabase) {
      ...
      if (!error && data) {
        const mapped = data.map((o: any) => normalizeOrder({
          id: o.id,
          userId: o.user_id || o.customer_id,
          userEmail: o.user_email,
          userName: o.user_name,
...
app.post("/api/orders", async (req, res) => {
  ...
  // ── 2. DEDUCT INVENTORY STOCK AUTHORITATIVELY ──
  for (const verifiedItem of verification.items) {
    const dbProduct = productsStore.find((p: any) => p.id === verifiedItem.id);
    if (dbProduct && dbProduct.stockQty !== undefined) {
      dbProduct.stockQty = Math.max(0, Number(dbProduct.stockQty) - verifiedItem.qty);
      if (isSupabaseConfigured) {
  ...
  // ── 3. ASSEMBLE NEW ORDER WITH STRICT AUTHORITATIVE TOTALS ──
  ...
  notificationsStore.unshift({
    ...
    body: `Your order #${newOrder.id} of ₹${newOrder.total} was placed with verified pricing. We're packing your fresh harvest now.`,
    type: 'order',
    read: false,
    createdAt: new Date().toISOString()
  });
...
const handleOrderUpdate = async (req: express.Request, res: express.Response) => {
  ...
  if (isSupabaseConfigured) {
    await syncOrderToSupabase(targetOrder);
  }
  ...
  persistAll();
  res.json({ success: true, order: targetOrder, orders: ordersStore.map(normalizeOrder) });
};
...
app.post("/api/payments/upi/initiate", async (req, res) => {
  try {
    ...
    // Authoritative Server-side Pricing & Stock Verification
    const verification = verifyCartAndCalculatePricing(items, couponCode, userId, userEmail, phone);
    if (!verification.valid) {
